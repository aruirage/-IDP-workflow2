const MAIN_BUILD = '683-zh-ui-translations';
const WF_CANONICAL_LAYOUT_KEY = 'neosai-idp-wf-canonical-layout-v27';
const appOptions = {
  setup() {
    
    const storedUiLanguage = localStorage.getItem('neosai-idp-ui-language');
    const uiLanguage = ref(storedUiLanguage === 'zh' ? 'zh' : 'ja');
    ...
    if (!form.processing.fraudDetect) {
      form.processing.fraudDetect = normalizeFraudDetectConfig(
        cloneJson(WORKFLOW_DEFAULTS.fraudDetect),
        form.scene.documents.map((d) => d.type),
      );
    }
    if (form.processing.qrRead) delete form.processing.qrRead;
    ...
    const readModelSettingsVisible = ref(false);
    const readModelHitlRules = [
      { value: 'low_confidence', label: '信頼度が閾値未満' },
      { value: 'multi_model_mismatch', label: '多モデル結果不一致' },
    ...
    const sceneSetupHeavyUiReady = ref(false);
    ...
    const UI_ZH_REPLACEMENTS = [
      ...
      ['マスク設定は帳票タイプ Step4 へ移行予定です（本期対象だが要件未確定のため、当面追加しません）。対象帳票は Step1 関連帳票から選択できます。', '掩码设定预定迁移至账票类型 Step4（本期要做但需求未明确，故暂不加）。对象账票可从 Step1 关联账票中选择。'],
      ...
      ['案件集約が完了したときに Workflow を開始します。開始ノードに設定項目はありません。', '案件集约完成时启动 Workflow。开始节点没有配置项。'],
      ...
      ['Step2の設定チェック完了後、ステータスが「公開可能」に更新され、公開できます。', 'Step2 设定检查完成后，状态会更新为「可发布」，即可发布。'],
      ...
      ['Photoshop等の編集痕跡、再圧縮、異常エッジなど改ざん痕跡を検出します。', '检测 Photoshop 等编辑痕迹、再压缩、异常边缘等篡改痕迹。'],
      ...
      ['分割時に QR 解码串から取る位置（1 起）。同一 QR ソース内で重複不可。', '分隔时从 QR 解码串取值的位置（从 1 起）。同一 QR 源内不可重复。'],
      ...
      ['ファイルが本案へ帰属し書類が揃ったとき、Workflow を起動または続行', '文件归属本案且资料齐全时，启动或续跑 Workflow'],
      ...
      ['OCR設定で定義した項目タイプを表示します。QR読取では編集できません。', '显示 OCR 设定中定义的项目类型。QR 读取时不可编辑。'],
      ...
      ['各モジュールを ON にし、対象帳票を指定（未指定時は関連帳票すべて）', '开启各模块，并指定对象账票（未指定时为全部关联账票）'],
      ...
      ['ON：区切り文字と順序を入力し、指定した 1 段だけを取得します。', 'ON：输入分隔符与顺序，仅取指定的 1 段。'],
      ...
      ['補件がアップロードされ本案へ帰属したとき、既存インスタンスを続行', '补件上传并归属本案时，续跑既有实例'],
      ['passed（通過）/ reviewRequired（要確認）', 'passed（通过）/ reviewRequired（要确认）'],
      ['QR を検出できませんでした。「QRスキャン」で再試行できます', '未能检测 QR。可用「QR扫描」重试'],
      ['Workflow の終点です。設定項目と出力変数はありません。', 'Workflow 终点。没有配置项和输出变量。'],
      ...
      ['区切り文字と順序を入力し、指定した 1 段だけを取得します', '输入分隔符与顺序，仅取指定的 1 段'],
      ...
      ['帳票の版面・テンプレート特徴との不一致や欠落を検出します。', '检测账票版面/模板特征不一致或缺失。'],
      ['列の読み取り結果が空の場合は、確認画面で赤く表示されます。', '列读取结果为空时，在确认画面以红色显示。'],
      ['无需配置；AI 检证入口等待汇总，人工待办 open 追加', '无需配置；AI 检证入口等待汇总，人工待办 open 追加'],
      ['string：文字列、number：数値、enum：列挙', 'string：字符串、number：数值、enum：枚举'],
      ['バッチ1・バッチ2 を順次取り込み（計 4 原ファイル）', '依次导入批次1・批次2（共 4 个原文件）'],
      ['を検出できませんでした。「QRスキャン」で再試行できます', '未能检测。「QR扫描」可重试'],
      ...
      ['脱敏対象帳票（未指定時は Step1 関連帳票すべて）', '脱敏对象账票（未指定时为 Step1 全部关联账票）'],
      ...
      ['Step1 でテンプレートをアップロードしてください', '请在 Step1 上传模板'],
      ['この項目は自動ルールマッチングと読取に使用されます。', '该项目用于自动规则匹配与读取。'],
      ...
      ['開始ノード：ゼロ設定（出力変数のみ下部に表示）', '开始节点：零配置（输出变量仅在下方显示）'],
      ...
      ['この帳票間関連のフィールドを追加してください', '请添加该账票间关联的字段'],
      ...
      ['当該列を人工確認の対象にするかを指定します。', '指定是否将该列作为人工确认对象。'],
      ...
      ['システム変数・ノード出力定義を挿入できます', '可插入系统变量・节点输出定义'],
      ['の終点です。設定項目と出力変数はありません', '终点。没有配置项和输出变量'],
      ['解码全文をそのままフィールドに書き込みます', '将解码全文原样写入字段'],
      ...
      ['AI生成画像や合成の痕跡を検出します。', '检测 AI 生成图像或合成相关痕迹。'],
      ...
      ['クリックでノード追加、ドラッグで接続', '点击添加节点，拖拽连接'],
      ...
      ['Step1 で関連帳票がありません', 'Step1 没有关联账票'],
      ['すべての帳票が主帳票へ到達できます', '全部账票均可到达主账票'],
      ...
      ['処理ルール後の値の正常範囲を判定。', '判定处理规则后的值是否在正常范围内。'],
      ...
      ['人工確認（ワークフローノード', '人工确认（工作流节点'],
      ['対象帳票（未指定時は全帳票）', '对象账票（未指定时为全部账票）'],
      ['帳票タイプを選択してください', '请选择账票类型'],
      ['追加できるノードがありません', '没有可添加的节点'],
      ['通知方式・宛先を指定します。', '指定通知方式和对象。'],
      ['復核ロールを選択してください', '请选择复核角色'],
      ['要素構造は実行時結果で決まる', '元素结构由运行时结果决定'],
      ['例：医療保険（通院給付）請求', '例：医疗保险（门诊给付）申请'],
      ...
      ['テンプレートをダウンロード', '下载模板'],
      ['案件集約時の画像分類信頼度', '案件集约时的图像分类置信度'],
      ['案件番号を入力してください', '请输入案件编号'],
      ['手動補件紐付け（案件詳細）', '手动补件关联（案件详情）'],
      ['出力をドラッグして回流接続', '拖拽输出进行回流连接'],
      ...
      ['関連プレビュー表示操作', '关联预览显示操作'],
      ['跨批次 + OCR缓冲', '跨批次 + OCR 缓冲'],
      ['跨批次上传（集約済み）', '跨批次上传（集约済み）'],
      ['公開ステータスについて', '关于发布状态'],
      ['再実行なし（次工程へ）', '不重新执行（进入下一工程）'],
      ...
      ['接続線をクリックで選択', '点击连接线选择'],
      ['待確認内容を表示します', '显示待确认内容'],
      ['置換後のテキストを入力', '输入替换后的文本'],
      ['置換前のテキストを入力', '输入替换前的文本'],
      ['帳票タイプ設定ステップ', '账票类型设定步骤'],
      ['帳票別Sheetで出力', '按账票输出 Sheet'],
      ['変数参照・分岐型不一致', '变量引用・分支类型不一致'],
      ['補件ファイル帰属で続行', '补件文件归属后续跑'],
      ['葉項目を選んでください', '请选择叶子字段'],
      ...
      ['件', '件'],
      ['種', '种'],
      ['補件：前述ノードへドラッグして回流接続', '补件：拖拽到前述节点做回流连接'],
      ['完了：クリックで追加、ドラッグで接続', '完成：点击添加，拖拽连接'],
      ['案件中止：クリックで追加、ドラッグで接続', '案件中止：点击添加，拖拽连接'],
      ['：クリックで追加、ドラッグで接続', '：点击添加，拖拽连接'],
      ['クリックで追加、ドラッグで接続', '点击添加，拖拽连接'],
    ];
    ...
    let uiTranslationTimer = null;
    const UI_ZH_EXACT_ONLY = new Set([
      '設定',
      '選択',
      '入力',
      '出力',
    ...
    const UI_ZH_REPLACEMENTS_SORTED = [...UI_ZH_REPLACEMENTS]
      .filter(([ja]) => !!ja)
      .sort((a, b) => b[0].length - a[0].length);
    const UI_ZH_EXACT_MAP = new Map(UI_ZH_REPLACEMENTS_SORTED);
    const UI_ZH_SUBSTRING_REPLACEMENTS = UI_ZH_REPLACEMENTS_SORTED
      .filter(([ja]) => !UI_ZH_EXACT_ONLY.has(ja));
    const UI_ZH_HINTS = UI_ZH_REPLACEMENTS_SORTED
      .map(([, zh]) => zh)
      .filter(Boolean);
    ...
    function translateUiString(value) {
      ...
      // 无日文假名/汉字时跳过，避免对每个文本节点做 900+ 次替换
      if (!/[\u3040-\u30ff\u4e00-\u9fff]/.test(result)) return result;
      const trimmed = result.trim();
      if (UI_ZH_EXACT_MAP.has(trimmed)) {
        const zh = UI_ZH_EXACT_MAP.get(trimmed);
        if (result === trimmed) return zh;
        return result.replace(trimmed, zh);
      }
      // 短键仅精确匹配；长句/短语才做子串替换，避免「システム设定」式半翻
      for (let i = 0; i < UI_ZH_SUBSTRING_REPLACEMENTS.length; i += 1) {
        const [ja, zh] = UI_ZH_SUBSTRING_REPLACEMENTS[i];
        if (result.includes(ja)) result = result.split(ja).join(zh);
      }
      return result;
    }
    ...
    function isLikelyTranslatedUiString(value) {
      ...
      return UI_ZH_HINTS.some((zh) => text.includes(zh));
    }
    ...
    function scheduleApplyUiLanguage() {
      if (uiLanguage.value !== 'zh') return;
      if (uiTranslationScheduled) return;
      uiTranslationScheduled = true;
      // 防抖：避免 Vue 重渲染 / MutationObserver 在挂载期连环触发全量翻译卡死主线程
      if (uiTranslationTimer) clearTimeout(uiTranslationTimer);
      uiTranslationTimer = setTimeout(() => {
        uiTranslationTimer = null;
    ...
    const fixedDocTypeSearch = ref('');
    const fixedDocTypeListCollapsed = ref(false);
    const fixedDocCustomTypes = reactive([]);
    let fixedDocCustomTypeSeq = 1;
    const FIXED_DOC_DEFAULT_TYPE = '診断書';
    function getFixedDocActiveTypeId() {
      return fixedDocSettingsTarget.value || FIXED_DOC_DEFAULT_TYPE;
    }
    function getFixedDocActiveDocLabel() {
      return getDocDisplayLabel(getFixedDocActiveTypeId());
    }
    const fixedDocSteps = [
      { id: 1, label: 'AI分類設定' },
      { id: 2, label: 'AI読取設定' },
    ...
    const fixedDocTypeId = computed(() => {
      const base = getFixedDocActiveTypeId();
      let hash = 0;
      String(base).split('').forEach((ch) => {
        hash = (hash * 31 + ch.charCodeAt(0)) % 10000000000000000;
      });
      return String(2080100000000000000n + BigInt(hash)).slice(0, 19);
    });
    const FIXED_DOC_DIAGNOSIS_PREVIEW_IMAGE = 'assets/mock-diagnosis-liaj045.png';
    const FIXED_DOC_HUMAN_DOCK_PREVIEW_IMAGE = 'assets/mock-human-dock.png';
    const fixedDocPreviewImage = computed(() => (
      getFixedDocActiveTypeId() === '人間ドック'
        ? FIXED_DOC_HUMAN_DOCK_PREVIEW_IMAGE
        : isFixedDocQrCapableDocType()
          ? FIXED_DOC_DIAGNOSIS_PREVIEW_IMAGE
          : ''
    ));
    const fixedDocPreviewScale = ref(1);
    const fixedDocPreviewRotate = ref(0);
    const fixedDocPreviewPanX = ref(0);
    ...
    function setFixedDocPreviewScale(nextScale, origin) {
      ...
      const scale = Math.min(3, Math.max(0.35, +nextScale.toFixed(2)));
      if (scale === previousScale) return;
      if (origin) {
        const offsetX = origin.clientX - origin.rect.left - origin.rect.width / 2 - fixedDocPreviewPanX.value;
        const offsetY = origin.clientY - origin.rect.top - origin.rect.height / 2 - fixedDocPreviewPanY.value;
        const ratio = scale / previousScale;
        fixedDocPreviewPanX.value = +(fixedDocPreviewPanX.value - offsetX * (ratio - 1)).toFixed(1);
        fixedDocPreviewPanY.value = +(fixedDocPreviewPanY.value - offsetY * (ratio - 1)).toFixed(1);
      }
      fixedDocPreviewScale.value = scale;
    }
    function fixedDocPreviewZoomIn() {
      setFixedDocPreviewScale(fixedDocPreviewScale.value + 0.15);
    }
    function fixedDocPreviewZoomOut() {
      setFixedDocPreviewScale(fixedDocPreviewScale.value - 0.15);
    }
    function fixedDocPreviewRotateLeft() {
      fixedDocPreviewRotate.value = (fixedDocPreviewRotate.value - 90 + 360) % 360;
    }
    function fixedDocPreviewRotateRight() {
      fixedDocPreviewRotate.value = (fixedDocPreviewRotate.value + 90) % 360;
    }
    function fixedDocPreviewReset() {
      fixedDocPreviewScale.value = 1;
      fixedDocPreviewRotate.value = 0;
      fixedDocPreviewPanX.value = 0;
      fixedDocPreviewPanY.value = 0;
    }
    function fixedDocPreviewFit() {
      fixedDocPreviewReset();
    }
    function onFixedDocPreviewWheel(event) {
      if (!fixedDocPreviewImage.value) return;
      const direction = event.deltaY > 0 ? -1 : 1;
      const rect = event.currentTarget.getBoundingClientRect();
      setFixedDocPreviewScale(fixedDocPreviewScale.value + direction * 0.12, {
    ...
    function normalizeFixedDocQrRegionRect(rect) {
      ...
      const y = clampFixedDocPreviewPercent(rect.y);
      const maxW = Math.max(minSize, 100 - x);
      const maxH = Math.max(minSize, 100 - y);
      return {
        x: +x.toFixed(2),
        y: +y.toFixed(2),
        w: +Math.max(minSize, Math.min(maxW, rect.w)).toFixed(2),
    ...
    function onFixedDocPreviewPointerDown(event) {
      ...
      fixedDocPreviewPanStart.panX = fixedDocPreviewPanX.value;
      fixedDocPreviewPanStart.panY = fixedDocPreviewPanY.value;
    ...
    function onFixedDocQrMappingRowClick(row, event) {
      if (event.target.closest('input, textarea, button, .el-select, .el-switch')) return;
      if (row.qrSourceId) focusFixedDocQrSource(row.qrSourceId);
    }
    function toggleFixedDocQrSourceEnabled(sourceId) {
      const src = fixedDocQrSourceCatalog.find((item) => item.id === sourceId);
      if (!src) return;
      src.enabled = src.enabled === false;
      if (src.enabled !== false && fixedDocActiveQrSourceId.value === sourceId) {
        fixedDocActiveQrSourceId.value = sourceId;
      }
      if (src.enabled === false && fixedDocActiveQrSourceId.value === sourceId) {
    ...
    function renameFixedDocQrSource(sourceId, label) {
      const src = fixedDocQrSourceCatalog.find((item) => item.id === sourceId);
      if (!src) return;
      src.label = String(label || '').slice(0, 40);
    }
    function fixedDocQrSourceDisplayLabel(src) {
      return src?.id || '';
    }
    function fixedDocQrSourceStatusLabel(src) {
      if (!src?.rect) return '未定位';
    ...
    function reindexFixedDocQrSources(removedSourceId = '') {
      const idMap = {};
      fixedDocQrSourceCatalog.forEach((source, index) => {
        const nextId = `QR${index + 1}`;
        idMap[source.id] = nextId;
        source.id = nextId;
      });
      updateFixedDocQrSourceMappingRefs(idMap, removedSourceId);
    ...
    function pushFixedDocQrIgnoredDetection(label, rect, idPrefix = 'DET', sourceKey = '') {
      if (!rect) return;
      const key = String(sourceKey || '').trim();
      if (key && fixedDocQrIgnoredDetections.some((item) => (item.sourceKey || item.sourceId) === key)) return;
      fixedDocQrIgnoredDetections.push({
    ...
    function registerFixedDocQrSourceFromDetection(detectionId, options = {}) {
      ...
      const detection = fixedDocQrIgnoredDetections[detectionIndex];
      const sourceId = options.sourceId || getNextFixedDocQrSourceId();
      const sourceKey = options.sourceKey || detection.sourceKey || detection.sourceId || '';
      const source = {
        id: sourceId,
        sourceKey: sourceKey || sourceId,
        label: options.label || detection.label || '新規 QR',
        enabled: true,
        rect: { ...detection.rect },
        manualRegion: false,
      };
      fixedDocQrSourceCatalog.push(source);
    ...
    function removeFixedDocQrSource(sourceId) {
      ...
      if (index < 0) return;
      const removed = fixedDocQrSourceCatalog[index];
      pushFixedDocQrIgnoredDetection(
        removed.label,
        removed.rect,
        'EXC',
        removed.sourceKey || removed.id,
      );
      fixedDocQrSourceCatalog.splice(index, 1);
      reindexFixedDocQrSources(sourceId);
      if (fixedDocActiveQrSourceId.value === sourceId) {
        fixedDocActiveQrSourceId.value = '';
      }
      ElementPlus.ElMessage.success(`${sourceId} を QRソースから除外しました`);
    }
    function listFixedDocQrIgnoredDetectionsOrdered() {
      return [...fixedDocQrIgnoredDetections].sort((a, b) => {
        const ax = a.rect?.x ?? 999;
        const bx = b.rect?.x ?? 999;
        if (ax !== bx) return ax - bx;
        return (a.rect?.y ?? 0) - (b.rect?.y ?? 0);
      });
    }
    function appendNextFixedDocQrSourceFromIgnored() {
      const next = listFixedDocQrIgnoredDetectionsOrdered()[0];
      if (!next) {
        ElementPlus.ElMessage.info('追加できる QR ソースがありません');
        return;
      }
      addFixedDocQrSourceFromIgnored(next.id);
    }
    function addFixedDocQrSourceFromIgnored(detectionId) {
    ...
    function assignFixedDocQrRegionRect(sourceId, rect) {
      ...
      source.rect = normalizeFixedDocQrRegionRect(rect);
    ...
    function assignFixedDocQrRegionFromDetection(sourceId, detectionId) {
      ...
      const detection = fixedDocQrIgnoredDetections[detectionIndex];
      source.rect = { ...detection.rect };
      source.manualRegion = true;
      source.enabled = true;
      if (!source.label || source.label === '新規 QR') {
    ...
    const fixedDocPreviewTitle = computed(() => getFixedDocActiveDocLabel());
    const fixedDocTypeItems = computed(() => [
      ...fixedDocCustomTypes,
      DOC_TYPE_REGISTRY.find((item) => item.id === '診断書'),
      DOC_TYPE_REGISTRY.find((item) => item.id === '人間ドック'),
    ].filter(Boolean));
    const fixedDocTypeItemsFiltered = computed(() => {
      const q = fixedDocTypeSearch.value.trim();
      if (!q) return fixedDocTypeItems.value;
      return fixedDocTypeItems.value.filter((item) => item.id.includes(q));
    });
    const fixedDocDescription = computed(() => {
      const doc = fixedDocPreviewTitle.value;
      if (doc.includes('診断書')) {
        return '入院・手術等証明書（診断書・LIAJ045-A01-202104）：上行結腸癌の診断、腹腔鏡下結腸悪性腫瘍切除術、入院期間、病理所見、医療機関情報を抽出する。';
      }
    ...
    const fixedDocReadTables = reactive([
      {
        ...
        name: '診療費明細',
        expanded: true,
        extractCondition: '調剤明細書のテーブルのみを抽出し、レセプトのテーブルは抽出しない。調剤明細書のテーブルが存在しない場合は、何も抽出しない。',
        columns: [
          {
            no: 1,
            columnId: 'read-table-1-col-1',
            key: '診療日',
            type: 'string',
            prompt: '診療日欄の日付を YYYY-MM-DD 形式で出力。',
            sample: '2025-09-08',
            required: false,
            hitl: true,
          ...
          {
            no: 2,
            columnId: 'read-table-1-col-2',
            key: '項目名',
            type: 'string',
            prompt: '区分欄の最左に記載されている項目名のみを出力。薬品名や点数は含めない。',
            sample: '調剤技術料',
            required: false,
            hitl: true,
          ...
          {
            no: 6,
            columnId: 'read-table-1-col-6',
            key: '調剤日',
            type: 'string',
            prompt: '調剤日欄の日付を YYYY-MM-DD 形式で出力。',
            sample: '2025-09-10',
            required: false,
            hitl: true,
            mask: false,            qrSourceId: 'QR2',
    ...
    /** Step2 HITL：ON=命中人工确认规则时要确认；OFF=跳过人工确认；默认 ON */
    const fixedDocFieldHitlByType = reactive({});
    function getFixedDocHitlBucket(typeId = getFixedDocActiveTypeId()) {
      const key = typeId || FIXED_DOC_DEFAULT_TYPE;
      if (!fixedDocFieldHitlByType[key]) {
        fixedDocFieldHitlByType[key] = {};
      }
    ...
    function getFixedDocFieldNamesForDocType(typeId) {
      ...
      const preferred = ['請求番号', '患者氏名', '被保険者氏名', '生年月日', '傷病名', '診断名', 'ICD10コード', '通院日数', '治療期間', '先進医療の有無', '通院日', '医療機関名', '医療機関住所', '医師名', '発行日'];
      const ordered = [...preferred.filter((f) => fields.includes(f)), ...fields.filter((f) => !preferred.includes(f))];
      return ordered.slice(0, 13);
    }
    function getFixedDocStableFieldIdForType(fieldName, typeId) {
      const names = getFixedDocFieldNamesForDocType(typeId);
      const baseIndex = names.indexOf(fieldName);
      return getFixedDocFieldId(fieldName, baseIndex);
    }
    function ensureFixedDocFieldHitlForType(typeId = getFixedDocActiveTypeId()) {
      const bucket = getFixedDocHitlBucket(typeId);
    ...
    /** マスク改至 Step4（本期要做，需求未明确故先不加）；敏感情报对象暂按 Step1 关联账票 */
    function docTypeHasMaskEnabled(typeId) {
      if (!typeId) return false;
      return getFixedDocFieldNamesForDocType(typeId).length > 0;
    }
    const fixedDocFieldTypes = reactive({});
    const fixedDocFieldOrder = ref([]);
    const fixedDocDeletedFieldNames = reactive(new Set());
    const fixedDocTextFieldDragId = ref('');
    ...
    function getFixedDocFieldId(fieldName, index = -1) {
      ...
      return `field-${index >= 0 ? index + 1 : slug}`;
    }
    function getFixedDocBaseFieldNameList() {
      const activeType = fixedDocTypeItems.value.find((item) => item.id === getFixedDocActiveTypeId());
      const fields = activeType?.fields || getDocSchema(fixedDocPreviewTitle.value).fields || getDocSchema('診断書').fields || [];
      const preferred = ['請求番号', '患者氏名', '被保険者氏名', '生年月日', '傷病名', '診断名', 'ICD10コード', '通院日数', '治療期間', '先進医療の有無', '通院日', '医療機関名', '医療機関住所', '医師名', '発行日'];
      const ordered = [...preferred.filter((f) => fields.includes(f)), ...fields.filter((f) => !preferred.includes(f))];
      return ordered.slice(0, 13);
    }
    ...
    function getFixedDocStableFieldId(fieldName) {
      const baseIndex = getFixedDocBaseFieldNameList().indexOf(fieldName);
      return getFixedDocFieldId(fieldName, baseIndex);
    }
    function getFixedDocFieldMeta(fieldRef) {
      return fixedDocTextRows.value.find((row) => row.fieldId === fieldRef || row.name === fieldRef) || null;
    }
    function getFixedDocFieldName(fieldRef) {
      return getFixedDocFieldMeta(fieldRef)?.name || fieldRef;
    }
    function getFixedDocFieldIds() {
      return getFixedDocFieldNameList().map((name) => getFixedDocStableFieldId(name));
    }
    function ensureFixedDocFieldHitl() {
      ensureFixedDocFieldHitlForType(getFixedDocActiveTypeId());
    }
    function toggleFixedDocFieldHitl(fieldRef, enabled) {
    ...
    function normalizeFixedDocFieldRuleForType(fieldRef) {
      ...
      if (!fixedDocFieldRules[fieldId]) return;
      normalizeFixedDocFieldType(fixedDocFieldTypes[fieldId] || meta?.type);
    }
    function deleteFixedDocTextField(fieldRef) {
      const meta = getFixedDocFieldMeta(fieldRef);
      if (!meta) return;
      fixedDocDeletedFieldNames.add(meta.name);
      fixedDocFieldOrder.value = fixedDocFieldOrder.value.filter((name) => name !== meta.name);
    }
    function reorderFixedDocTextField(dragFieldId, targetFieldId) {
      if (!dragFieldId || !targetFieldId || dragFieldId === targetFieldId) return;
      const rows = fixedDocTextRows.value;
      const dragRow = rows.find((row) => row.fieldId === dragFieldId);
      const targetRow = rows.find((row) => row.fieldId === targetFieldId);
      if (!dragRow || !targetRow) return;
      const names = getFixedDocFieldNameList().filter((name) => name !== dragRow.name);
      const targetIndex = names.indexOf(targetRow.name);
      names.splice(targetIndex < 0 ? names.length : targetIndex, 0, dragRow.name);
      fixedDocFieldOrder.value = names;
    }
    function moveFixedDocTextField(fieldRef, direction) {
      const meta = getFixedDocFieldMeta(fieldRef);
      if (!meta) return;
      const names = getFixedDocFieldNameList();
      const currentIndex = names.indexOf(meta.name);
      if (currentIndex < 0) return;
      const nextIndex = direction === 'up' ? currentIndex - 1 : currentIndex + 1;
      if (nextIndex < 0 || nextIndex >= names.length) return;
      const nextNames = [...names];
      const [moved] = nextNames.splice(currentIndex, 1);
      nextNames.splice(nextIndex, 0, moved);
      fixedDocFieldOrder.value = nextNames;
    }
    function onFixedDocTextFieldDragStart(fieldId, event) {
      fixedDocTextFieldDragId.value = fieldId;
      fixedDocTextFieldDropTargetId.value = '';
      event.dataTransfer?.setData('text/plain', fieldId);
      if (event.dataTransfer) event.dataTransfer.effectAllowed = 'move';
    }
    function onFixedDocTextFieldDragOver(fieldId, event) {
      if (!fixedDocTextFieldDragId.value || fixedDocTextFieldDragId.value === fieldId) return;
      fixedDocTextFieldDropTargetId.value = fieldId;
      if (event?.dataTransfer) event.dataTransfer.dropEffect = 'move';
    }
    function onFixedDocTextFieldDrop(fieldId, event) {
      const dragFieldId = fixedDocTextFieldDragId.value || event?.dataTransfer?.getData('text/plain') || '';
      reorderFixedDocTextField(dragFieldId, fieldId);
      fixedDocTextFieldDragId.value = '';
      fixedDocTextFieldDropTargetId.value = '';
    }
    function onFixedDocTextFieldDragEnd() {
      fixedDocTextFieldDragId.value = '';
      fixedDocTextFieldDropTargetId.value = '';
    }
    const fixedDocHitlStats = computed(() => {
      ensureFixedDocFieldHitl();
      const ids = getFixedDocFieldIds();
      const bucket = getFixedDocHitlBucket();
      const hitlOn = ids.filter((fieldId) => bucket[fieldId] !== false).length;
      return { total: ids.length, hitlOn };
    });
    const fixedDocTextRows = computed(() => {
      const samples = {
        患者氏名: '安達　珠美',
        被保険者氏名: '安達　珠美',
        性別: '女',
        ...
        傷病名: '上行結腸癌',
        ICD10コード: 'C182',
        発病又は受傷年月日: '2026年5月18日',
        主たる手術等: '腹腔鏡下結腸悪性腫瘍切除術',
        入院年月日: '2026年6月11日',
        退院年月日: '2026年6月19日',
        手術年月日: '2026年6月12日',
        診断名: 'Adenocarcinoma of the ascending colon',
        通院日数: '20',
        発行日: '2026年6月25日',
    ...
    const fixedDocRangeModeOptions = [
    ...
    const fixedDocQrTemplate = reactive({
      templateId: 'LIAJ045-A01-202104',
      templateLabel: '生命保険診断書（LIAJ045）',
    });
    ...
    const FIXED_DOC_QR_SOURCE_DEFAULTS = [
      ...
      { id: 'QR2', label: '傷病・通院', enabled: true },
      { id: 'QR3', label: '手術・入院', enabled: true },
    ...
    const FIXED_DOC_QR_SAMPLE_PAYLOADS = {
      ...
      QR3: '血陽性$はしば内科クリニック$$1$$$$腹腔鏡下結腸悪性腫瘍切除術$1$$7190300$2026$6$12$$$$$$$$$$$$$$$$$$$$1$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$',
      QR4: '$$$$$$$$$$$$$$$$$$$$$$1$Adenocarcinoma of the ascending colon$2026$5$23$$1$$$$$2026$5$20$進行大腸癌$$1$3$1$0$$1$$1$1$1$$$上行結',
      QR5: '腸腫瘍$$$1$$$$$$$$$$$$$$$$$$$$1$$$$$$$$$$$$$$$$$$$$1$$$$$$$$$$$$$$$$$$$$1$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$1$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$',
    ...
    const FIXED_DOC_QR_FIELD_PRESET = {
      ...
      性別: { qrSourceId: 'QR1', fetchIndex: 3 },
      カルテ番号: { qrSourceId: 'QR1', fetchIndex: 4 },
      生年月日: { qrSourceId: 'QR1', fetchIndex: 5 },
      傷病名: { qrSourceId: 'QR1', fetchIndex: 8 },
      ICD10コード: { qrSourceId: 'QR1', fetchIndex: 9 },
      発病又は受傷年月日: { qrSourceId: 'QR2', fetchIndex: 3 },
      主たる手術等: { qrSourceId: 'QR3', fetchIndex: 7 },
      入院年月日: { qrSourceId: 'QR3', fetchIndex: 11 },
      退院年月日: { qrSourceId: 'QR3', fetchIndex: 13 },
      診断名: { qrSourceId: 'QR4', fetchIndex: 23 },
      手術年月日: { qrSourceId: 'QR3', fetchIndex: 11 },
      発行日: { qrSourceId: 'QR6', fetchIndex: 48 },
      医療機関名: { qrSourceId: 'QR6', fetchIndex: 53 },
      診療科: { qrSourceId: 'QR6', fetchIndex: 54 },
    };
    ...
    function getFixedDocFieldQrExtractMethod(row = {}) {
      return normalizeFixedDocQrExtractMethod(row.qrExtractMethod || row.qrDetail?.extractMethod);
    }
    function isFixedDocQrSplitExtractMethod(methodOrRow) {
      const method = typeof methodOrRow === 'string'
        ? normalizeFixedDocQrExtractMethod(methodOrRow)
        : getFixedDocFieldQrExtractMethod(methodOrRow);
      return method === FIXED_DOC_QR_EXTRACT_SPLIT;
    }
    function isFixedDocQrPlainExtractMethod(methodOrRow) {
      const method = typeof methodOrRow === 'string'
        ? normalizeFixedDocQrExtractMethod(methodOrRow)
        : getFixedDocFieldQrExtractMethod(methodOrRow);
      return method === FIXED_DOC_QR_EXTRACT_PLAIN;
    }
    function splitFixedDocQrPreserveEmpty(text, delimiter) {
      const sep = delimiter == null ? '$' : String(delimiter);
      const raw = String(text ?? '');
      if (!raw) return [''];
      return raw.split(sep);
    }
    function formatFixedDocQrSplitPreviewToken(token, emptyToken) {
      const value = String(token ?? '');
      if (value === '') return '(空)';
      if (emptyToken && value === emptyToken) return '(空)';
      return value;
    }
    function getFixedDocQrSourceDefinition(sourceId) {
      return fixedDocQrSourceCatalog.find((item) => item.id === sourceId) || null;
    }
    function getFixedDocQrSamplePayload(sourceId) {
      const source = getFixedDocQrSourceDefinition(sourceId);
      return FIXED_DOC_QR_SAMPLE_PAYLOADS[source?.sourceKey || sourceId] || '';
    }
    function resolveFixedDocQrFieldConfig(fieldRule = {}) {
      const qrSourceId = fieldRule.qrSourceId || fieldRule.sourceQr;
      if (!qrSourceId) return null;
      const fetchIndex = Number.isFinite(Number(fieldRule.fetchIndex)) ? Number(fieldRule.fetchIndex) : 0;
      return {
        qrSourceId,
        fetchIndex,
        delimiter: fieldRule.qrDelimiter || fieldRule.qrDetail?.delimiter || '$',
        emptyToken: fieldRule.qrDetail?.emptyToken || '-',
        extractMethod: getFixedDocFieldQrExtractMethod(fieldRule),
        failureRule: fieldRule.qrDetail?.failureRule || '空文字を出力',
      };
    }
    function buildFixedDocQrSplitPreview(sourceId, delimiter, emptyToken) {
      const sep = delimiter || '$';
      const token = emptyToken ?? '-';
      const parts = splitFixedDocQrPreserveEmpty(getFixedDocQrSamplePayload(sourceId), sep);
      return parts.map((part, index) => ({
        index: index + 1,
        display: formatFixedDocQrSplitPreviewToken(part, token),
        raw: part,
    ...
    function getFixedDocQrSourceFieldRows(sourceId) {
      return fixedDocTextRows.value.filter((row) => {
        ...
        return rule.qrSourceId === sourceId && isFixedDocQrSplitExtractMethod(rule);
      });
    }
    function createDefaultFixedDocQrDetail() {
      return {
        extractMethod: FIXED_DOC_QR_EXTRACT_SPLIT,
        delimiter: '$',
        emptyToken: '-',
        failureRule: '空文字を出力',
      };
    }
    const fixedDocFieldRules = reactive({});
    const fixedDocRuleSelection = reactive({});
    const fixedDocBatchRule = ref('');
    const fixedDocMaskSelection = reactive({});
    ...
    function clearAllFixedDocConfiguredQrFieldMappings() {
      ...
      fixedDocTextRows.value.forEach((row) => {
        ...
        setFixedDocFieldRule(fieldId, {
          qrSourceId: '',
          sourceQr: '',
          qrExtractMethod: '',
          qrDelimiter: '',
          fetchIndexManual: false,
        });
    ...
    function getFixedDocFieldRangeRule(row) {
      return row.rule || '';
    }
    function shouldShowFixedDocFieldQrMapping(fieldName, row = {}) {
      return isFixedDocQrCapableDocType();
    }
    function shouldShowFixedDocQrFieldSplitIndex(row = {}) {
      if (!row.qrSourceId) return false;
      return isFixedDocQrSplitExtractMethod(row);
    }
    function shouldShowFixedDocQrFieldDelimiter(row = {}) {
      return shouldShowFixedDocQrFieldSplitIndex(row);
    }
    function shouldShowFixedDocQrFieldExtractMethod(row = {}) {
      return !!row.qrSourceId;
    }
    function getNextFixedDocQrFetchIndex(sourceId, excludeFieldId = '') {
      if (!sourceId) return 0;
      ensureFixedDocFieldRules();
      const used = new Set();
      getFixedDocQrSourceFieldRows(sourceId).forEach((row) => {
        if (row.fieldId === excludeFieldId) return;
        const rule = fixedDocFieldRules[row.fieldId] || {};
        if (Number.isFinite(Number(rule.fetchIndex))) used.add(Number(rule.fetchIndex));
      });
      let index = 0;
      while (used.has(index)) index += 1;
      return index;
    }
    function getFixedDocQrFetchIndexOneBased(row, fieldId = row?.fieldId) {
      if (!fieldId) return null;
      if (fieldId && fixedDocQrFetchIndexDrafts[fieldId] !== undefined) {
        const trimmed = String(fixedDocQrFetchIndexDrafts[fieldId]).trim();
        if (!trimmed) return null;
        const parsed = Number(trimmed);
        return Number.isInteger(parsed) ? parsed : null;
      }
      ensureFixedDocFieldRules();
      const rule = fixedDocFieldRules[fieldId] || row || {};
      if (!Number.isFinite(Number(rule.fetchIndex))) return null;
      return Number(rule.fetchIndex) + 1;
    }
    function validateFixedDocQrFetchIndex(sourceId, fieldId, rawValue) {
      const trimmed = String(rawValue ?? '').trim();
      if (!trimmed) return { ok: false, message: '順序を入力してください' };
      const oneBased = Number(trimmed);
      if (!Number.isInteger(oneBased) || oneBased < 1) {
        return { ok: false, message: '1以上の整数を入力してください' };
      }
    ...
    function getFixedDocQrFetchIndexStoredDisplay(row) {
      ...
      if (!Number.isFinite(Number(rule.fetchIndex))) return '';
      return String(Number(rule.fetchIndex) + 1);
    }
    function getFixedDocQrFetchIndexInput(row) {
      if (!row?.fieldId) return '';
      if (fixedDocQrFetchIndexDrafts[row.fieldId] !== undefined) {
        return fixedDocQrFetchIndexDrafts[row.fieldId];
      }
    ...
    function commitFixedDocQrFetchIndexDraft(fieldId) {
      const draft = fixedDocQrFetchIndexDrafts[fieldId];
      ...
      const result = validateFixedDocQrFetchIndex(sourceId, fieldId, draft);
      if (!result.ok) {
        fixedDocQrFetchIndexErrors[fieldId] = result.message;
        fixedDocQrFetchIndexDrafts[fieldId] = draft;
        return;
      }
      delete fixedDocQrFetchIndexErrors[fieldId];
      setFixedDocFieldRule(fieldId, { fetchIndex: result.value, fetchIndexManual: true });
    ...
    function ensureFixedDocQrFetchIndexes(sourceId) {
      ...
      getFixedDocQrSourceFieldRows(sourceId).forEach((row) => {
        const rule = fixedDocFieldRules[row.fieldId];
        if (!rule) return;
        if (!Number.isFinite(Number(rule.fetchIndex))) {
          rule.fetchIndex = getNextFixedDocQrFetchIndex(sourceId, row.fieldId);
        }
      });
    }
    function toggleFixedDocFieldQrMapping(fieldRef, enabled) {
    ...
    function migrateFixedDocQrFieldRules() {
      ensureFixedDocFieldRules();
      fixedDocTextRows.value.forEach((row) => {
        const current = fixedDocFieldRules[row.fieldId];
        if (current.rule !== FIXED_DOC_QR_RULE_PRESET) return;
        setFixedDocFieldRule(row.fieldId, {
          rule: current.ocrFallbackRule || getFixedDocPostProcessRule(row.name) || 'OCR読取',
          qrSourceId: current.qrSourceId || '',
          fetchIndex: current.fetchIndex ?? getFixedDocFieldQrPreset(row.name)?.fetchIndex ?? 0,
          sourceQr: current.qrSourceId || '',
          qrExtractMethod: current.qrExtractMethod || FIXED_DOC_QR_EXTRACT_SPLIT,
          qrDelimiter: current.qrDelimiter || '$',
        });
      });
    }
    function canFixedDocRuleHaveNormalCondition(rule = '') {
      const normalized = String(rule || '').trim();
      if (!normalized) return false;
      return normalized !== 'マスタ照合' && normalized !== 'テキスト置換';
    }
    function getFixedDocRangeModeOptionsByRule(rule = '') {
      if (!canFixedDocRuleHaveNormalCondition(rule)) return [];
      return fixedDocRangeModeOptions;
    }
    function canFixedDocFieldHaveRange(row) {
      return canFixedDocRuleHaveNormalCondition(row?.rule);
    }
    ...
    function normalizeFixedDocNormalConditionForRule(target = {}, rule = '') {
      if (!canFixedDocRuleHaveNormalCondition(rule)) {
    ...
    function formatFixedDocRangeSummary(row) {
      ...
      const rangeRule = row.rule || '';
      if (!row.rangeEnabled) return '';
      const mode = row.rangeMode || 'between';
      if (mode === 'enum') return row.normalValues || '列挙値未設定';
      const min = row.rangeMin || '—';
      const max = row.rangeMaxToday ? '当日' : (row.rangeMax || '—');
      if (mode === 'lt') return `< ${max}`;
      if (mode === 'lte') return `≤ ${max}`;
    ...
    function inferFixedDocAiMatchingRule(fieldName) {
      ...
      if (/電話|携帯|氏名|住所|申告|性別|被保険|フリガナ|口座|金融|支店|預金|通帳|登録|送付|親権|名義/.test(fieldName)) return '';
      if (/番号|個人番号|マイナンバー|証券/.test(fieldName)) return '数字のみ';
      if (/日|記入日|発行|証明|調剤|生年月/.test(fieldName)) return '日付';
      if (/金額|保険金|請求額|合計/.test(fieldName)) return '金額（JPY）';
      if (/点数|日数|数量|回数/.test(fieldName)) return '数字のみ';
      if (/フリガナ|カナ/.test(fieldName)) return 'カタカナ';
      if (/率|身長|体重|比例/.test(fieldName)) return '小数点付きの数字';
      return '';
    }
    ...
    function ensureFixedDocTableColumnRangeFields(col) {
      col.type = normalizeFixedDocFieldType(col.type);
      if (col.rangeEnabled === undefined) col.rangeEnabled = false;
      if (col.rangeMode === undefined) col.rangeMode = 'between';
      if (col.rangeMin === undefined) col.rangeMin = '';
      if (col.rangeMax === undefined) col.rangeMax = '';
      if (col.rangeMaxToday === undefined) col.rangeMaxToday = false;
    ...
    function setFixedDocTableColumnRule(tableId, columnRef, patch) {
      ...
      if (patch.rule !== undefined) {
        normalizeFixedDocNormalConditionForRule(col, col.rule);
      }
    }
    ...
    function buildFixedDocTableAiMatchingDraft() {
      return getFixedDocTableColumnEntries().map(({ tableId, tableName, col }) => {
        ensureFixedDocTableColumnRangeFields(col);
        const rule = inferFixedDocAiMatchingRule(col.key) || col.rule || '';
        return {
          tableId,
          tableName,
          columnId: col.columnId,
          fieldKey: col.key,
          name: `${tableName} / ${col.key}`,
          type: col.type,
          rule,
        };
      });
    }
    function applyFixedDocAiMatchingDraft(drafts) {
      drafts.forEach((draft) => {
        setFixedDocFieldRule(draft.fieldId || draft.name, {
          rule: draft.rule,
        });
      });
    }
    function applyFixedDocTableAiMatchingDraft(drafts) {
      drafts.forEach((draft) => {
        setFixedDocTableColumnRule(draft.tableId, draft.columnId, {
          rule: draft.rule,
        });
      });
    }
    function isFixedDocRulesEmpty() {
      ensureFixedDocFieldRules();
      return fixedDocTextRows.value.every((row) => !fixedDocFieldRules[row.fieldId]?.rule);
    }
    function isFixedDocTableRulesEmpty() {
      return getFixedDocTableColumnEntries().every(({ col }) => !col.rule);
    }
    function runFixedDocAiMatching() {
      if (fixedDocRuleTab.value === 'table') {
        applyFixedDocTableAiMatchingDraft(buildFixedDocTableAiMatchingDraft());
      } else {
        applyFixedDocAiMatchingDraft(buildFixedDocAiMatchingDraft());
      }
      ElementPlus.ElMessage.success('AIマッチングを反映しました');
    }
    function bootstrapFixedDocQrFieldRules() {
      migrateFixedDocQrFieldRules();
    }
    function autoApplyFixedDocAiMatchingIfNeeded() {
      if (fixedDocSetupStep.value !== 3) return;
      bootstrapFixedDocQrFieldRules();
      if (isFixedDocRulesEmpty()) {
        applyFixedDocAiMatchingDraft(buildFixedDocAiMatchingDraft());
      }
      if (isFixedDocTableRulesEmpty()) {
        applyFixedDocTableAiMatchingDraft(buildFixedDocTableAiMatchingDraft());
      }
    }
    function getFixedDocPostProcessRule(fieldName) {
      const ruleMap = {
        生年月日: '日付',
        入院年月日: '日付',
        退院年月日: '日付',
    ...
    function isFixedDocQrCapableDocType() {
    ...
    function ensureFixedDocRuleSelection() {
      getFixedDocFieldIds().forEach((fieldId) => {
        if (!Object.prototype.hasOwnProperty.call(fixedDocRuleSelection, fieldId)) {
          fixedDocRuleSelection[fieldId] = false;
        }
      });
    }
    function toggleFixedDocRuleSelection(fieldRef, selected) {
      ensureFixedDocRuleSelection();
      const fieldId = getFixedDocFieldMeta(fieldRef)?.fieldId || fieldRef;
      fixedDocRuleSelection[fieldId] = !!selected;
    }
    function toggleFixedDocRuleSelectAll(selected) {
      ensureFixedDocRuleSelection();
      getFixedDocFieldIds().forEach((fieldId) => {
        fixedDocRuleSelection[fieldId] = !!selected;
      });
    }
    const fixedDocRuleSelectionCount = computed(() => {
      ensureFixedDocRuleSelection();
      return getFixedDocFieldIds().filter((fieldId) => fixedDocRuleSelection[fieldId]).length;
    });
    const fixedDocRuleSelectAll = computed({
      get() {
        ensureFixedDocRuleSelection();
        const ids = getFixedDocFieldIds();
        return ids.length > 0 && ids.every((fieldId) => fixedDocRuleSelection[fieldId]);
      },
      set(value) {
        toggleFixedDocRuleSelectAll(value);
      },
    });
    function applyFixedDocBatchRule() {
      if (!fixedDocBatchRule.value) return;
      ensureFixedDocRuleSelection();
      fixedDocTextRows.value.forEach((row) => {
        if (!fixedDocRuleSelection[row.fieldId]) return;
        setFixedDocFieldRule(row.fieldId, { rule: fixedDocBatchRule.value });
      });
    }
    function ensureFixedDocMaskSelection() {
      getFixedDocFieldIds().forEach((fieldId) => {
        if (!Object.prototype.hasOwnProperty.call(fixedDocMaskSelection, fieldId)) {
          fixedDocMaskSelection[fieldId] = false;
        }
      });
    }
    function toggleFixedDocMaskSelection(fieldRef, selected) {
      ensureFixedDocMaskSelection();
      const fieldId = getFixedDocFieldMeta(fieldRef)?.fieldId || fieldRef;
      fixedDocMaskSelection[fieldId] = !!selected;
    }
    function toggleFixedDocMaskSelectAll(selected) {
      ensureFixedDocMaskSelection();
      getFixedDocFieldIds().forEach((fieldId) => {
        fixedDocMaskSelection[fieldId] = !!selected;
      });
    }
    const fixedDocMaskSelectionCount = computed(() => {
      ensureFixedDocMaskSelection();
      return getFixedDocFieldIds().filter((fieldId) => fixedDocMaskSelection[fieldId]).length;
    });
    const fixedDocMaskSelectAll = computed({
      get() {
        ensureFixedDocMaskSelection();
        const ids = getFixedDocFieldIds();
        return ids.length > 0 && ids.every((fieldId) => fixedDocMaskSelection[fieldId]);
      },
      set(value) {
        toggleFixedDocMaskSelectAll(value);
      },
    });
    function applyFixedDocBatchHitl(enabled) {
      ensureFixedDocMaskSelection();
      fixedDocTextRows.value.forEach((row) => {
        if (!fixedDocMaskSelection[row.fieldId]) return;
        toggleFixedDocFieldHitl(row.fieldId, enabled);
      });
    }
    function getDefaultFixedDocFieldRule(fieldName) {
      const qrMap = getFixedDocFieldQrPreset(fieldName);
      ...
      const base = {
        ...
        qrDetail: createDefaultFixedDocQrDetail(),
        master: fieldName === '医療機関名'
          ? '病院名・住所-病院名'
          : /傷病名|診断名/.test(fieldName)
            ? '傷病名・ICD10-傷病名'
            : fieldName === 'ICD10コード'
              ? '傷病名・ICD10-ICD10'
              : '',
    ...
    function getDefaultFixedDocMaster(fieldName) {
      ...
      if (/傷病名|診断名/.test(fieldName)) return '傷病名・ICD10-傷病名';
      if (fieldName === 'ICD10コード') return '傷病名・ICD10-ICD10';
      return '銀行名・支店名・コード';
    }
    function isFixedDocMasterRule(row) {
      return row?.rule === 'マスタ照合';
    }
    function isFixedDocTextReplaceRule(row) {
    ...
    function canFixedDocTableColumnHaveNormalCondition(col) {
      return canFixedDocRuleHaveNormalCondition(col?.rule);
    }
    function getFixedDocNormalConditionMode(row) {
      return row?.rangeEnabled ? (row.rangeMode || 'between') : '';
    }
    function getFixedDocTableColumnNormalConditionMode(col) {
      return col?.rangeEnabled ? (col.rangeMode || 'between') : '';
    }
    function getFixedDocNormalConditionModeOptions(row) {
      return getFixedDocRangeModeOptionsByRule(row?.rule);
    }
    function getFixedDocTableColumnNormalConditionModeOptions(col) {
      return getFixedDocRangeModeOptionsByRule(col?.rule);
    }
    function hasFixedDocRuleParams(row) {
      return isFixedDocMasterRule(row) || isFixedDocTextReplaceRule(row) || canFixedDocRowHaveNormalCondition(row);
    }
    function ensureFixedDocFieldRules() {
      fixedDocTextRows.value.forEach((row) => {
        if (!fixedDocFieldRules[row.fieldId]) {
          fixedDocFieldRules[row.fieldId] = {
            ...(fixedDocFieldRules[row.name] || getDefaultFixedDocFieldRule(row.name)),
            fieldId: row.fieldId,
          };
        }
      });
    }
    function setFixedDocFieldRule(fieldRef, patch) {
      ensureFixedDocFieldRules();
      const meta = getFixedDocFieldMeta(fieldRef);
      const fieldId = meta?.fieldId || fieldRef;
      const fieldName = meta?.name || fieldRef;
      if (!fixedDocFieldRules[fieldId]) {
        fixedDocFieldRules[fieldId] = getDefaultFixedDocFieldRule(fieldName);
      }
      ...
      if (patch.rule !== undefined) {
        normalizeFixedDocNormalConditionForRule(
          fixedDocFieldRules[fieldId],
          fixedDocFieldRules[fieldId].rule,
        );
      }
      if (patch.qrSourceId) {
        fixedDocFieldRules[fieldId].sourceQr = patch.qrSourceId;
        if (!fixedDocFieldRules[fieldId].qrExtractMethod) {
          fixedDocFieldRules[fieldId].qrExtractMethod = FIXED_DOC_QR_EXTRACT_SPLIT;
          fixedDocFieldRules[fieldId].qrDelimiter = fixedDocFieldRules[fieldId].qrDelimiter || '$';
        }
        focusFixedDocQrSource(patch.qrSourceId);
      }
      if (patch.qrExtractMethod) {
      ...
      if (patch.sourceQr) {
        fixedDocFieldRules[fieldId].qrSourceId = patch.sourceQr;
      }
      ...
      if (patch.qrExtractMethod === FIXED_DOC_QR_EXTRACT_SPLIT && fixedDocFieldRules[fieldId].qrSourceId) {
        ensureFixedDocQrFetchIndexes(fixedDocFieldRules[fieldId].qrSourceId);
      }
    }
    ...
    const fixedDocQrNumberOptions = computed(() => fixedDocQrSourceCatalog.map((slot) => ({
      value: slot.id,
      label: slot.id,
    })));
    ...
    function sortFixedDocQrSourceCatalog() {
      fixedDocQrSourceCatalog.sort((a, b) => {
        ...
        const nb = Number(String(b.id).replace(/\D/g, '')) || 999;
        return na - nb;
      });
    }
    function listFixedDocQrTemplateDetections() {
      return Object.keys(FIXED_DOC_QR_SOURCE_RECTS)
        .map((key) => {
          const def = FIXED_DOC_QR_SOURCE_DEFAULTS.find((item) => item.id === key) || {
            id: key,
            label: key,
            enabled: true,
          };
          return {
            sourceKey: key,
            label: def.label,
            rect: { ...FIXED_DOC_QR_SOURCE_RECTS[key] },
          };
        })
        .sort((a, b) => a.rect.x - b.rect.x || a.rect.y - b.rect.y);
    }
    function getFixedDocQrExcludedSourceKeys() {
      return new Set(
        fixedDocQrIgnoredDetections
          .map((item) => item.sourceKey || item.sourceId)
          .filter(Boolean),
      );
    }
    function applyFixedDocQrSourcesFromTemplate(options = {}) {
      ...
      for (let index = fixedDocQrIgnoredDetections.length - 1; index >= 0; index -= 1) {
        const key = fixedDocQrIgnoredDetections[index].sourceKey || fixedDocQrIgnoredDetections[index].sourceId;
        if (key && !scanKeys.has(key)) fixedDocQrIgnoredDetections.splice(index, 1);
      }

      fixedDocQrSourceCatalog.splice(0, fixedDocQrSourceCatalog.length);
      detections.forEach((detection) => {
        if (excludedKeys.has(detection.sourceKey)) {
          const existing = fixedDocQrIgnoredDetections.find(
            (item) => (item.sourceKey || item.sourceId) === detection.sourceKey,
          );
          if (existing) {
            existing.rect = { ...detection.rect };
            existing.label = detection.label;
            existing.sourceKey = detection.sourceKey;
            existing.sourceId = detection.sourceKey;
          } else {
            pushFixedDocQrIgnoredDetection(detection.label, detection.rect, 'EXC', detection.sourceKey);
          }
          return;
        }
        fixedDocQrSourceCatalog.push({
          id: `QR${fixedDocQrSourceCatalog.length + 1}`,
    ...
    async function runFixedDocQrSourcesScan(options = {}) {
      ...
      if (options.clearFieldMappings) {
        clearAllFixedDocConfiguredQrFieldMappings();
      }
      fixedDocQrSourceCatalog.splice(0, fixedDocQrSourceCatalog.length);
      fixedDocActiveQrSourceId.value = '';
    ...
    function parseFixedDocQrSourcesFromTemplate() {
      if (fixedDocHasConfiguredFieldQrMapping.value) {
        ElementPlus.ElMessageBox.confirm(
          '既にフィールド映射が設定されています。QRスキャンを再実行すると、フィールド映射がクリアされ、QRソース目録が上書きされます。続行しますか？',
          'QRスキャン',
          {
          ...
          runFixedDocQrSourcesScan({ clearFieldMappings: true });
        }).catch(() => {});
        return;
      }
    ...
    function runFixedDocOcrAiGenerate() {
      ElementPlus.ElMessage.success('OCR promptと正解サンプルを AI 生成しました');
    }
    function runFixedDocAiGenerate() {
      runFixedDocOcrAiGenerate();
    }
    function exportFixedDocFieldTemplate() {
      ElementPlus.ElMessage.info('項目・テーブル定義テンプレートをエクスポートしました（プロトタイプ）');
    }
    function maskFixedDocPreviewValue(fieldRef, value) {
    ...
    function evaluateFixedDocTestRange(row, value) {
      ...
      const buildDetail = (override = {}) => {
        ...
        return formatFixedDocRangeSummary(summaryRow) || '';
      };
      if (row.name === '通院日数') {
        const detail = buildDetail({
          rangeMode: row.rangeMode || 'between',
          rangeMin: row.rangeMin || '1',
    ...
    const fixedDocQrEffectTestSourceId = ref('QR1');
    const fixedDocQrEffectSplitPreview = computed(() => (
      buildFixedDocQrSplitPreview(fixedDocQrEffectTestSourceId.value, '$', '-')
    ));
    const fixedDocQrEffectPlainPreview = computed(() => (
      getFixedDocQrSamplePayload(fixedDocQrEffectTestSourceId.value)
    ));
    const fixedDocQrEffectTestUsesSplit = ref(true);
    const fixedDocImageRuleGroups = [
      {
        title: '画像分割',
        targetLabel: '画像分割対象',
        enabled: false,
        items: [{ label: '分割判定閾値', kind: 'number', value: 80, min: 0, max: 100 }],
      },
      {
        title: '画像組合',
        info: true,
        ...
        enabled: false,
        items: [
          { label: '組合必須キー', kind: 'select', value: '', options: ['証明日', '医療機関名', '患者氏名'] },
          { label: '組合任意キー', kind: 'select', value: '', options: ['ページ番号', '連番', '帳票管理番号'] },
        ],
      },
      {
        title: '画像整列',
        ...
        enabled: false,
        items: [
          { label: '整列キー', kind: 'select', value: '', options: ['日付', 'ページ番号', '連番'] },
          { label: '整列キータイプ', kind: 'radio', value: '日付', options: ['日付', '数値'] },
          { label: '整列方向', kind: 'radio', value: '昇順', options: ['昇順', '降順'] },
        ],
      },
    ];
    ...
    function handleFixedDocTemplateAction(command) {
      ...
      const message = command === 'batch-import'
        ? '一括インポートを開始しました'
        : '一括ダウンロードを開始しました';
      ElementPlus.ElMessage.success(message);
    }
    function handleFixedDocSettingsAction(command) {
      if (command === 'read-model-settings') {
        openReadModelSettings();
        return;
      }
      ElementPlus.ElMessage.success('エクスポート設定を開きました');
    }
    ...
    function openFixedDocSettings(typeId, stepOverride, options = {}) {
      fixedDocSettingsTarget.value = typeId ? typeId : '';
      const docLabel = getFixedDocActiveDocLabel();
    ...
    function openFixedDocSettingsFromMenu() {
      const firstType = fixedDocTypeItems.value[0];
      if (firstType?.id) {
        openFixedDocSettings(firstType.id, 1);
        return;
      }
      switchModule('fixed-doc');
    }
    function createFixedDocType() {
      const id = fixedDocCustomTypeSeq === 1 ? '体检报告' : `体检报告${fixedDocCustomTypeSeq}`;
      fixedDocCustomTypeSeq += 1;
      fixedDocCustomTypes.unshift({
        id,
        category: 'custom',
        icon: '新',
        fields: ['項目名1', '項目名2', '項目名3'],
      });
      fixedDocTypeSearch.value = '';
      openFixedDocSettings(id, 1);
    }
    function closeFixedDocSettings() {
      switchModule('case-workflow');
    }
    function goFixedDocPrevStep() {
      fixedDocSetupStep.value = Math.max(1, fixedDocSetupStep.value - 1);
    }
    function goFixedDocNextStep() {
    ...
    function saveFixedDocSettings() {
      if (revalidateAllFixedDocQrFetchIndexes()) {
        ElementPlus.ElMessage.warning('QR区切り設定の入力を確認してください');
        return;
      }
    ...
    function getWorkflowNotificationEventOptions(nodeType) {
      ...
      const statusKeys = [vars.status].filter(Boolean);
      const statusOptions = statusKeys.flatMap((key) =>
        WORKFLOW_NOTIFICATION_STATUS_VALUES.map((value) => ({
          value: `${key}:${value}`,
          label: `${key} = ${value}`,
        })));
      const resultKeys = [vars.result].filter(Boolean);
      const resultOptions = resultKeys.flatMap((key) =>
    ...
    function migrateWorkflowNotificationPolicies(policies) {
      ...
      WORKFLOW_NOTIFICATION_NODE_OPTIONS.forEach((node) => {
        ...
        getWorkflowNotificationEventOptions(node.value).forEach((event) => {
          ...
          const emailRecipients = normalizeNotifyEmailRecipients(policy?.emailRecipients);
          if (!recipients.length && !emailRecipients && !policy?.subject && !policy?.body) return;
          rules.push({
            id: newRuleId('ntf'),
            nodeType: node.value,
            event: event.value,
            channels: normalizeNotifyChannels(policy?.channels),
            recipients,
            emailRecipients,
    ...
    function normalizeWorkflowNotificationRule(rule = {}) {
      ...
      return {
        ...
        nodeType,
        event,
        channels: normalizeNotifyChannels(rule.channels),
        recipients,
        emailRecipients: normalizeNotifyEmailRecipients(rule.emailRecipients),
    ...
    const workflowNotificationRuleRows = computed(() =>
      syncWorkflowNotificationStore().map((rule, index) => ({
        ...
        channelLabels: formatNotifyChannelsDisplay(rule.channels),
        recipientLabels: rule.recipients
          .map((value) => NOTIFY_RECIPIENT_OPTIONS.find((item) => item.value === value)?.label)
          .filter(Boolean),
        emailRecipientDisplay: formatNotifyEmailRecipientsDisplay(rule.emailRecipients),
    ...
    const filteredWorkflowNotificationRuleRows = computed(() => {
      ...
      return workflowNotificationRuleRows.value.filter((rule) =>
        [rule.subject, rule.body, rule.emailRecipientDisplay].some((value) => String(value || '').toLowerCase().includes(query))
      );
    });
    ...
    function bindNotifyChannelMinSelection(getChannels, setChannels) {
      let lastValid = parseNotifyChannelSelection(getChannels());
      if (!lastValid.length) lastValid = ['system'];
      watch(getChannels, (channels) => {
        const selected = parseNotifyChannelSelection(channels);
        if (!selected.length) {
          setChannels([...lastValid]);
          ElementPlus.ElMessage.warning('通知チャネルを1つ以上選択してください');
          return;
        }
        lastValid = selected;
      }, { deep: true });
    ...
    bindNotifyChannelMinSelection(
      () => notificationRuleDraft.channels,
      (channels) => { notificationRuleDraft.channels = channels; },
    );
    bindNotifyChannelMinSelection(
      () => notificationRuleEditDraft.channels,
      (channels) => { notificationRuleEditDraft.channels = channels; },
    );
    ...
    function validateWorkflowNotificationRule(rule, options = {}) {
      ...
      const channels = parseNotifyChannelSelection(rule.channels);
      if (!channels.length) return '通知チャネルを1つ以上選択してください';
      if (channels.includes('system') && (!Array.isArray(rule.recipients) || !rule.recipients.length)) {
        return '通知対象を選択してください';
      }
      if (channels.includes('email')) {
        const emailResult = validateNotifyEmailRecipients(rule.emailRecipients);
    ...
    function saveWorkflowNotificationRuleDraft() {
      const err = validateWorkflowNotificationRule(notificationRuleDraft, { excludeId: notificationRuleDraft.id });
      if (err) {
    ...
    function saveWorkflowNotificationRuleEdit() {
      const err = validateWorkflowNotificationRule(notificationRuleEditDraft, { excludeId: notificationRuleEditDraft.id });
      if (err) {
        ElementPlus.ElMessage.warning(err);
        return;
      }
    ...
    const sceneSetupNetworkLayout = computed(() => {
      if (!sceneSetupHeavyUiReady.value) {
        return { width: 720, height: 320, nodes: [], edges: [] };
      }
      return buildSceneSetupNetworkLayout(
        sceneSetupDraft.documents,
    ...
    function getHitlGateBranchConnectTitle(branchKey) {
      ...
      if (action === 'request_supplement') return t('補件：前述ノードへドラッグして回流接続');
      return t(`${getHitlGateActionLabel(branchKey)}：クリックで追加、ドラッグで接続`);
    }
    ...
    function onNotifyChannelChange(channel) {
      const node = selectedWorkflowNode.value;
      if (!node || node.type !== 'notify') return;
      if (channel === 'system' || channel === 'email') {
    ...
    function isFraudDetectEnabled(typeId) {
      // 只读：禁止在 computed 渲染路径里写回 fraudDetect，否则会触发无限响应式刷新卡死 Step2
      const pf = processingForm.value;
      const types = form.scene.documents.map((d) => d.type);
      if (!types.includes(typeId)) return false;
      const enabled = Array.isArray(pf.fraudDetect?.enabledTypes) ? pf.fraudDetect.enabledTypes : [];
      if (!enabled.length) return true;
      return enabled.includes(typeId);
    }
    ...
    function ensurePiiMaskConfig() {
      const pf = processingForm.value;
      if (!pf.piiMask) {
        pf.piiMask = { enabledTypes: [] };
      }
      if (!Array.isArray(pf.piiMask.enabledTypes)) pf.piiMask.enabledTypes = [];
    }

    function isPiiMaskEnabled(typeId) {
      // 只读：禁止在 computed 渲染路径里写回 piiMask
      const pf = processingForm.value;
      const types = form.scene.documents.map((d) => d.type);
      if (!types.includes(typeId)) return false;
      const enabled = Array.isArray(pf.piiMask?.enabledTypes) ? pf.piiMask.enabledTypes : [];
      if (!enabled.length) return true;
      return enabled.includes(typeId);
    }

    function togglePiiMask(typeId, enabled) {
      ensurePiiMaskConfig();
      const pf = processingForm.value;
      const allTypes = form.scene.documents.map((d) => d.type);
      let enabledTypes = [...pf.piiMask.enabledTypes];
      if (!enabledTypes.length) enabledTypes = [...allTypes];
      if (enabled) {
        if (!enabledTypes.includes(typeId)) enabledTypes.push(typeId);
      } else {
      ...
      pf.piiMask.enabledTypes = enabledTypes;
    }

    const piiMaskItems = computed(() => form.scene.documents.map((d) => ({
      type: d.type,
      enabled: isPiiMaskEnabled(d.type),
    })));

    const piiMaskStats = computed(() => {
      const items = piiMaskItems.value;
      const enabledItems = items.filter((i) => i.enabled);
      return {
        total: items.length,
        enabled: enabledItems.length,
    ...
    const piiDetectEligibleDocTypes = computed(() =>
      sceneDocTypes.value.filter((typeId) => docTypeHasMaskEnabled(typeId)),
    );

    function syncPiiDetectDocTypesOnForm() {
      const image = processingForm.value?.image;
      if (!image) return;
      const eligible = piiDetectEligibleDocTypes.value;
      let selected = Array.isArray(image.piiDetectDocTypes)
        ? image.piiDetectDocTypes.filter((t) => eligible.includes(t))
        : [];
      if (image.piiDetect && !selected.length && eligible.length) {
        selected = [...eligible];
      }
      image.piiDetectDocTypes = selected;
    }

    function getPreprocessSettingDocTypes(item) {
      if (item?.key === 'piiDetect') return piiDetectEligibleDocTypes.value;
      return sceneDocTypes.value;
    }

    function getPreprocessDocTypePlaceholder(item) {
      if (item?.key === 'piiDetect') {
        return piiDetectEligibleDocTypes.value.length
          ? '脱敏対象帳票（未指定時は Step1 関連帳票すべて）'
          : 'Step1 で関連帳票がありません';
      }
      return '対象帳票（未指定時は全帳票）';
    }

    watch(
      [sceneDocTypes, () => processingForm.value?.image?.piiDetect],
      () => { syncPiiDetectDocTypesOnForm(); },
      { deep: true },
    );
    ...
    function buildWorkflowReferenceRoutedPath(draft, obstacles, routedSegments, edgeIndex) {
      ...
      // 限制 XY 笛卡尔积，避免节点多时边路由候选爆炸卡死 Step2 渲染
      const xs = [...candidateXs];
      const ys = [...candidateYs];
      if (xs.length * ys.length <= 48) {
    ...
    const wfNodePickerAvailableProcessGroups = computed(() => {
        ...
        return CASE_FLOW_NODE_GROUPS.map((group) => ({
          ...
          nodes: [...group.nodes]
            .filter((item) => {
              ...
              return [item.label, t(item.label), getWorkflowNodePickerDescription(item.type)]
      ...
      const grouped = {};
      nodes.filter((item) => !query || [item.label, t(item.label), getWorkflowNodePickerDescription(item.type)]
        .some((text) => String(text || '').toLowerCase().includes(query))).forEach((item) => {
        const key = item.category || '処理';
        if (!grouped[key]) grouped[key] = [];
        grouped[key].push({ ...item, label: t(item.label) });
      });
      return Object.entries(grouped).map(([category, groupNodes]) => ({ category: t(category), nodes: groupNodes }));
    });
    ...
    function loadSceneSetupDraftFromData(data, sceneId = '', sceneName = '') {
      ...
      if (!sceneSetupDraft.mainKey && sceneSetupDraft.mainDocType) {
        const fields = getSceneSetupFieldOptions(sceneSetupDraft.mainDocType);
        sceneSetupDraft.mainKey = fields.find((field) => /請求番号|証券番号|契約番号|案件番号/.test(field))
          || fields.find((field) => /(氏名|被保険者)/.test(field))
          || fields[0]
          || '';
      }
      if (sceneSetupDraft.documents.length >= 2) {
        ...
        if (linkErr || !sceneSetupDraft.docFieldLinks.length) {
    ...
    function proceedToWorkflowStep() {
      ...
      if (!persistSceneSetupDraft({ validate: true })) {
        
        return false;
      }
      sceneSetupVisible.value = false;
      // 先切到 Step2 并保持 inspector 折叠，避免与画布首帧同时挂载导致主线程卡死
      inspectorPanelCollapsed.value = true;
      workflowSetupStep.value = 2;
      nextTick(() => {
        fitWorkflowToView();
    ...
    function isSceneSetupAggregateDetailOpen(group) {
      ...
      // 默认折叠：完整默认关联会生成多组规则，全部展开会同步挂载大量 el-select 导致主线程卡死
    ...
    function toggleFixedDocTypeList() {
      fixedDocTypeListCollapsed.value = !fixedDocTypeListCollapsed.value;
    }
    ...
    function getWorkflowNodeCanvasSummary(node) {
      return t(buildWorkflowNodeCanvasSummary(node, getWorkflowCanvasSummaryContext()));
    }

    const localizedPreprocessSettingItems = computed(() =>
      PREPROCESS_SETTING_ITEMS.map((item) => ({
        ...item,
        label: t(item.label),
      }))
    );

    const localizedFraudDetectDetectorOptions = computed(() =>
      FRAUD_DETECT_DETECTOR_OPTIONS.map((item) => ({
        ...item,
        label: t(item.label),
        hint: t(item.hint || ''),
      }))
    ...
    const localizedHitlContextOptions = computed(() =>
      HITL_CONTEXT_OPTIONS.map((item) => ({ ...item, label: t(item.label) }))
    );
    const localizedHitlActionOptions = computed(() =>
      HITL_ACTION_OPTIONS.map((item) => ({ ...item, label: t(item.label) }))
    );
    const localizedGatewayConditionLogic = computed(() =>
      GATEWAY_CONDITION_LOGIC.map((item) => ({ ...item, label: t(item.label) }))
    );
    const localizedGatewayTimeoutStrategies = computed(() =>
      GATEWAY_TIMEOUT_STRATEGIES.map((item) => ({ ...item, label: t(item.label) }))
    );
    const localizedGatewayRerunPolicies = computed(() =>
      GATEWAY_RERUN_POLICIES.map((item) => ({ ...item, label: t(item.label) }))
    );
    const localizedCaseWorkflowStartTriggers = computed(() =>
      CASE_WORKFLOW_START_TRIGGERS.map((item) => ({
        ...item,
        label: t(item.label),
        detail: t(item.detail),
        actionLabel: t(item.actionLabel),
        caseStatus: t(item.caseStatus),
      }))
    );
    ...
    function ensureFraudDetectDetectors() {
      const image = processingForm.value?.image;
      if (!image) return normalizeFraudDetectDetectors(null, sceneDocTypes.value);
      const fallback = Array.isArray(image.fraudDetectDocTypes) && image.fraudDetectDocTypes.length
        ? image.fraudDetectDocTypes
        : sceneDocTypes.value;
      const next = normalizeFraudDetectDetectors(image.fraudDetectDetectors, fallback);
      if (JSON.stringify(next) !== JSON.stringify(image.fraudDetectDetectors || {})) {
        image.fraudDetectDetectors = next;
      }
      return image.fraudDetectDetectors || next;
    }

    function isFraudDetectDetectorOn(key) {
      const image = processingForm.value?.image;
      return isFraudDetectDetectorEnabled(image?.fraudDetectDetectors, key);
    }

    function getFraudDetectDetectorDocTypes(key) {
      const image = processingForm.value?.image;
      const docs = readFraudDetectDetectorDocTypes(image?.fraudDetectDetectors, key);
      if (docs.length) return docs.filter((t) => sceneDocTypes.value.includes(t));
      return isFraudDetectDetectorOn(key) ? [...sceneDocTypes.value] : [];
    }

    function setFraudDetectDetectorDocTypes(key, docTypes) {
      const detectors = ensureFraudDetectDetectors();
      if (!detectors[key]) {
        detectors[key] = normalizeFraudDetectDetectorEntry(undefined, sceneDocTypes.value);
      }
      detectors[key].docTypes = Array.isArray(docTypes)
        ? docTypes.filter((t) => sceneDocTypes.value.includes(t))
        : [];
      // 模块级列表同步为各二级选中账票并集，供画布摘要与旧逻辑兜底
      const image = processingForm.value?.image;
      if (image) {
        const union = new Set();
        Object.keys(detectors).forEach((detectorKey) => {
          if (!detectors[detectorKey]?.enabled) return;
          (detectors[detectorKey].docTypes || []).forEach((t) => union.add(t));
        });
        image.fraudDetectDocTypes = union.size ? [...union] : [...sceneDocTypes.value];
      }
    }

    function toggleFraudDetectDetector(key) {
      const detectors = ensureFraudDetectDetectors();
      if (!Object.prototype.hasOwnProperty.call(detectors, key)) return;
      const entry = normalizeFraudDetectDetectorEntry(detectors[key], sceneDocTypes.value);
      entry.enabled = !entry.enabled;
      if (entry.enabled && !entry.docTypes.length) {
        entry.docTypes = [...sceneDocTypes.value];
      }
      detectors[key] = entry;
    }

    function togglePreprocessModule(item) {
      if (item.alwaysOn || !item.switchKey) return;
      const image = processingForm.value?.image;
      if (!image) return;
      image[item.switchKey] = !image[item.switchKey];
      if (item.key === 'piiDetect') syncPiiDetectDocTypesOnForm();
      if (item.key === 'fraudDetect' && image[item.switchKey]) {
        const detectors = ensureFraudDetectDetectors();
        const union = new Set();
        Object.keys(detectors).forEach((key) => {
          ...
          detectors[key] = entry;
          if (entry.enabled) entry.docTypes.forEach((t) => union.add(t));
        });
        image.fraudDetectDocTypes = union.size ? [...union] : [...sceneDocTypes.value];
      }
    }
    ...
    function showPreprocessSettingDetail(item) {
      if (item.alwaysOn) return true;
      if (item.key === 'fraudDetect') return isPreprocessSettingOn(item);
      if (!item.docTypesKey) return false;
    ...
    function getWorkflowNodeDisplayLabel(node) {
      if (!node) return '';
      return t(getWorkflowNodeMeta(node.type).title);
    }
    ...
    watch(workflowSetupStep, (step) => {
      if (step !== 2) return;
      // 延后打开 inspector / 选中开始节点，避免与 Step2 模板首屏抢主线程
      nextTick(() => {
        requestAnimationFrame(() => {
    ...
    watch(
      ...
      () => {
        ...
        const nextFraudDocs = filterImageDocTypes(img.fraudDetectDocTypes, allowed);
        const nextPiiDocs = filterImageDocTypes(img.piiDetectDocTypes, allowed);
        ...
        if (JSON.stringify(nextFraudDocs) !== JSON.stringify(img.fraudDetectDocTypes || [])) img.fraudDetectDocTypes = nextFraudDocs;
        if (JSON.stringify(nextPiiDocs) !== JSON.stringify(img.piiDetectDocTypes || [])) img.piiDetectDocTypes = nextPiiDocs;
        const nextDetectors = normalizeFraudDetectDetectors(img.fraudDetectDetectors, allowed);
        if (JSON.stringify(nextDetectors) !== JSON.stringify(img.fraudDetectDetectors || {})) {
          img.fraudDetectDetectors = nextDetectors;
        }
        ensureFraudDetectConfig();
        syncFraudDetectTypes();
    ...
    onMounted(() => {
      ...
      // 延后挂载关联预览网络图，避免与 Step1 首屏抢主线程
      sceneSetupHeavyUiReady.value = false;
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          sceneSetupHeavyUiReady.value = true;
        });
      });
      ...
      nextTick(() => {
        ...
        applyUiLanguage();
        let sceneSetupNetworkViewportRaf = 0;
        const updateSceneSetupNetworkViewport = () => {
          if (sceneSetupNetworkViewportRaf) return;
          sceneSetupNetworkViewportRaf = requestAnimationFrame(() => {
            sceneSetupNetworkViewportRaf = 0;
            const el = sceneSetupNetworkViewportRef.value;
            if (!el) return;
            const width = el.clientWidth || 0;
            const height = el.clientHeight || 0;
            const prevW = sceneSetupNetworkViewportSize.width;
            const prevH = sceneSetupNetworkViewportSize.height;
            if (width === prevW && height === prevH) return;
            // 忽略 fit-scale 反馈导致的小幅收缩级联，避免 ResizeObserver ↔ 缩放死循环卡死页面
            if (
              prevW > 0
              && prevH > 0
              && width <= prevW + 1
              && height < prevH
              && height >= Math.max(280, prevH - 48)
              && sceneSetupNetworkZoom.value == null
            ) {
              return;
            }
            sceneSetupNetworkViewportSize.width = width;
            sceneSetupNetworkViewportSize.height = height;
          });
        };
    ...
    return {
      ...
      HITL_ACTION_OPTIONS: localizedHitlActionOptions,
      HITL_CONTEXT_OPTIONS: localizedHitlContextOptions,
      HITL_PRESET_OPTIONS: localizedHitlContextOptions,
      inferHitlContext,
      getHitlContextMeta,
      getHitlGatePreset,
      isHitlGateNode,
      ...
      GATEWAY_CONDITION_LOGIC: localizedGatewayConditionLogic,
      GATEWAY_TIMEOUT_STRATEGIES: localizedGatewayTimeoutStrategies,
      GATEWAY_RERUN_POLICIES: localizedGatewayRerunPolicies,
      ...
      onSubmissionChange,
      openReadModelSettings,
      handleFixedDocTemplateAction,
      handleFixedDocSettingsAction,
      ...
      aiAssistDataRule,
      sceneDocTypes,
      piiDetectEligibleDocTypes,
      getPreprocessSettingDocTypes,
      getPreprocessDocTypePlaceholder,
      syncPiiDetectDocTypesOnForm,
      dataRuleCount,
      currentModule,
      inspectorExpanded,
      ...
      fixedDocTypeSearch,
      fixedDocTypeListCollapsed,
      toggleFixedDocTypeList,
      fixedDocTypeItemsFiltered,
      createFixedDocType,
      fixedDocSetupStep,
      fixedDocSteps,
      fixedDocReadTab,
      ...
      fixedDocPreviewRotateLeft,
      fixedDocPreviewRotateRight,
      fixedDocPreviewReset,
      fixedDocPreviewFit,
      onFixedDocPreviewWheel,
      ...
      addFixedDocQrSourceFromIgnored,
      appendNextFixedDocQrSourceFromIgnored,
      ...
      moveFixedDocTextField,
      onFixedDocTextFieldDragStart,
      onFixedDocTextFieldDragOver,
      onFixedDocTextFieldDrop,
      onFixedDocTextFieldDragEnd,
      ...
      fixedDocQrEffectTestUsesSplit,
      isFixedDocQrSplitExtractMethod,
      isFixedDocQrPlainExtractMethod,
      shouldShowFixedDocQrFieldSplitIndex,
      shouldShowFixedDocQrFieldDelimiter,
      ...
      setFixedDocQrSplitEnabled,
      getFixedDocFieldQrExtractMethod,
      getFixedDocQrFieldPreviewValue,
      getFixedDocQrSourceDefinition,
      fixedDocRuleSelectAll,
      fixedDocRuleSelectionCount,
      fixedDocBatchRule,
      fixedDocRuleSelection,
      toggleFixedDocRuleSelection,
      applyFixedDocBatchRule,
      fixedDocMaskSelectAll,
      fixedDocMaskSelectionCount,
      fixedDocMaskSelection,
      toggleFixedDocMaskSelection,
      applyFixedDocBatchHitl,
      setFixedDocFieldRule,
      setFixedDocNormalConditionMode,
      setFixedDocTableColumnNormalConditionMode,
      canFixedDocFieldHaveRange,
      getFixedDocFieldRangeRule,
      ...
      runFixedDocAiMatching,
      autoApplyFixedDocAiMatchingIfNeeded,
      fixedDocImageRuleGroups,
      fixedDocTestRows,
      fixedDocTableTestRows,
      ...
      openFixedDocSettingsFromMenu,
      closeFixedDocSettings,
      goFixedDocPrevStep,
      goFixedDocNextStep,
      saveFixedDocSettings,
      ...
      CASE_WORKFLOW_START_TRIGGERS: localizedCaseWorkflowStartTriggers,
      ...
      localizedPreprocessSettingItems,
      FRAUD_DETECT_DETECTOR_OPTIONS,
      localizedFraudDetectDetectorOptions,
      localizedHitlContextOptions,
      localizedHitlActionOptions,
      localizedGatewayConditionLogic,
      localizedGatewayTimeoutStrategies,
      localizedGatewayRerunPolicies,
      localizedCaseWorkflowStartTriggers,
      isPreprocessSettingOn,
      togglePreprocessModule,
      isFraudDetectDetectorOn,
      toggleFraudDetectDetector,
      getFraudDetectDetectorDocTypes,
      setFraudDetectDetectorDocTypes,
...
async function bootstrapApp() {
  ...
  try {
    app.mount('#app');
    const state = app._container?._vnode?.component?.setupState || app._instance?.setupState;
    const proxy = app._container?._vnode?.component?.proxy || app._instance?.proxy;
    if (proxy || state) {
