...
| 中文      | 日语/界面          | 定义                                                                                                                                               |
| ------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| 画像不正検知  | 画像不正検知         | 前处理节点内模块：检测 AI / 模版 / AI生成 三类异常；模型输出通过/不通过，不通过时须返回不正类型字段给后端；模块开关 + 对象账票                                                                                  |
| 不正类型    | 不正タイプ          | 画像不正検知判「不通过」时返回的枚举：AI / 模版 / AI生成；供后端与前处理人工确认页展示                                                                                  |
...
| 脱敏标记    | マスク            | Step2 OCR設定 字段/表格列开关：标记**该账票内**哪些字段/列为脱敏与打码对象（字段级）；仅定义范围，**不单独触发**运行时脱敏                                                                          |
| 敏感情报检出  | 敏感情報検出         | 前处理节点子模块：**模块开关 + 对象账票**（账票级）；对象账票须**同时**为业务场景 Step1 关联账票，且该账票 Step2 至少一项 マスク=ON（テキスト読取 字段或 テーブル情報読取 列）；**仅当**模块 ON 且实例账票在对象列表中，才读取该账票 マスク 配置并打码 |
...
| 敏感情报检出 vs 画像不正検知            | 脱敏为前处理工程（读取对象账票的 マスク 配置并打码）；画像不正为模型二值判定（通过/不通过）+ 不通过时返回不正类型（AI / 模版 / AI生成）→ 前处理人工确认待办；二者均无阈值配置                                                                                                                    |
| マスク vs 敏感情报检出               | **包含关系**：マスク（Step2）定**字段/列**；敏感情报检出（节点）定**账票**；对象账票可选范围 = **Step1 关联账票 ∩ Step2 存在 マスク=ON**（含表格列）；运行时 = 节点 ON + 实例账票在对象列表 → 读取该账票 マスク=ON 项并打码                                           |
...
```
配置：Step3 启用正常条件 + 読取モデル設定 人工確認ルール（**默认规则 6 + 7**，可改组合）→ Step5 test 验收
运行：后处理完成 → 正常条件比较 → 已启用规则 OR 判定 → reviewRequired → 我的任务 · OCR 抽出确认
```

1. admin 在 Step3 按 **処理ルール** 为字段/列选配正常条件（见 6.03 · 适用 処理ルール）；与 Step2 プロンプト（提示词）、処理ルール 对齐；Step5 展示字段来源标签（QR/OCR）。
2. 运行时逐字段比较后处理产物（**OCR、QR 写值、QR→OCR 兜底均适用**）；**仅评估读取模型弹窗中已勾选**的人工确认规则，**OR** 合并——**默认规则 6 + 7**：**QR 写值**越界由规则 6 命中；**OCR / 兜底**越界由规则 6 命中，或低置信且越界由规则 7 命中（规则 6 已覆盖全部越界场景）。若 admin 另启规则 1，则 OCR 低置信 alone 亦可进确认（组合见 6.04 · 多选配置）。
3. 操作员在执行页修正要確認字段；**不展示**范围结论、配置区间、读取来源或命中哪条规则；办结仍校验正常值（未通过则阻断）。

---

### 2.2 故事：マスク（敏感情报检出）

角色：admin（账票侧配置 マスク、节点侧启用敏感情报检出）；操作员（查看脱敏结果；无专用待办）

目标：**マスク** 在账票 Step2 标记字段级脱敏对象；**敏感情报检出** 在前处理节点以开关 + 对象账票决定账票级是否执行；运行时仅对「节点 ON 且对象账票命中」的实例，读取该账票 マスク=ON 配置并打码。

配置流程（两层，包含关系）：

```
账票 Step2：字段/列 マスク=ON（字段级，定义「哪些字段要脱敏」）
        ↓ 节点运行时读取
业务场景 Step2 前処理設定：敏感情报检出 ON + 对象账票（账票级，定义「哪些账票要脱敏」）
```

1. admin 在账票类型 Step2 OCR設定 下，对需脱敏的文本字段或表格列开启マスク（如患者氏名、住所；テキスト読取 / テーブル情報読取 均有マスク列）。
2. 在业务场景 Step2 前処理設定 开启「敏感情报检出」，从对象账票中选择要脱敏的账票。**可选范围须同时满足**：① 该业务场景 Step1 已关联；② 该账票 Step2 至少一项 マスク=ON（テキスト読取 字段或 テーブル情報読取 列）。
3. Step5 test 标记マスク対象；不展示脱敏后像素预览（遮挡预览在前处理确认页，见运行流程）。

运行流程：

```
前处理：画像不正検知 → 回転 → 補正 → 整列 → 敏感情报检出 → OCR 抽出读脱敏后实例
```

1. 工作流进入前处理节点；当敏感情报检出 ON 且当前实例账票在对象账票列表中，读取该账票 Step2 マスク=ON 的字段/列并执行脱敏与打码（在几何处理后执行）。
2. 脱敏成功 → 后续 OCR 抽出读取脱敏后实例；案件详情与导出字段值为脱敏后值。
3. 前处理确认页（已脱敏场景）：默认遮挡后；マスク=ON 字段值统一打码展示。
4. 节点 OFF、或未选该账票、或该账票无 マスク=ON 字段 → 跳过脱敏，OCR 读原实例。
5. 敏感情报检出模块执行失败 → preprocessStatus=failed，走案件异常；不新增脱敏专用待办；可并行收到 Step3 通知。
...
目标：在前处理节点启用画像不正検知，对指定账票检测 AI / 模版 / AI生成 三类异常；模型判「不通过」时返回不正类型并进人工确认（非 OCR 抽出确认）。
...
2. 依赖账票 Step1 template 与客户正常样张库做模版特征比对；AI / AI生成依赖开源算法（待确定）判断是否伪造。
3. 可选：Step3 为前处理对象配置 preprocessResult=reviewRequired 或 preprocessStatus=failed 的通知规则。

运行流程：

```
前处理 · 画像不正検知 → 模型输出「不通过」+ 不正类型 → preprocessResult=reviewRequired → 我的任务 · 人工确认
```

1. 前处理串行执行时，画像不正検知最先运行（在几何处理与脱敏之前）。
2. 模型二值输出「不通过」→ 须返回不正类型字段（枚举：AI / 模版 / AI生成）给后端；生成人工确认待办；操作员在人工确认页可见类型并选择完了/补件/案件中止。
3. 输出「通过」→ 不正类型为空；继续后续子模块（回転 → 補正 → 整列 → 敏感情报检出）与 OCR 抽出。
...
目标：在账票 Step2 字段读取 tab 配置 QR 源与字段映射；运行时按三种情况路由；与 OCR 路径汇合后统一七项确认规则。
...
2. **QR扫描**仅生成或刷新 QR 源目录，不自动写入字段映射。admin 在映射表逐字段**可选**配置 QRソース、取値方法、区切り文字、順序；**未选 QRソース 的字段运行时走 OCR**（OCR 模式维护項目名/タイプ/マスク/必须/字段排序/删减操作） 在 QR 模式只读）。テーブル情報読取 不参与 QR。
...
| 步骤  | 模块           | 理由                                       |
| --- | ------------ | ---------------------------------------- |
| 1   | 画像不正検知       | 优先在原实例上判定篡改/伪造风险；不通过时返回不正类型（AI / 模版 / AI生成） |
| 2～4 | 回転 → 補正 → 整列 | 几何预处理                                    |
| 5   | 敏感情报检出       | 几何完成后对对象账票读取 Step2 マスク 配置并打码；OCR 读脱敏后实例；无业务待办 |


文件状态四段枚举不变；子模块进度汇总至 preprocessStatus / preprocessResult，不在案件一覧展示子模块名。

1. 前处理面板：画像不正检测、敏感情报检出（各：开关 + 对象账票）；执行顺序为不正检测 → 回転/補正/整列 → 脱敏。
2. 敏感情报检出：模块 ON 且实例账票在对象列表时，读取该账票 Step2 マスク=ON 的字段/列并打码。对象账票可选范围须同时满足：① 业务场景 Step1 关联账票；② 该账票 Step2 至少一项 マスク=ON（含 テーブル情報読取 列）。
3. 画像不正检测：输出通过/不通过；判「不通过」时须返回不正类型字段（枚举：AI / 模版 / AI生成）给后端，并设 preprocessResult=reviewRequired → 人工确认。
...
| 不正类型（返回后端）                      | 不通过时必填；枚举 AI / 模版 / AI生成；通过时为空 |
...
| 正常值校验 | 账票 Step3 正常条件          | 后处理成功后比较（全路径）；规则 6：越界 alone；规则 7：OCR·兜底低置信且越界 → OCR 抽出确认（6.04）                        |
| 确认规则  | 读取模型弹窗七项               | 多选 OR；默认规则 6 + 7；运行时只评估已勾选规则；适用矩阵见 6.04                                            |
...
| ---- | ---------------- | -------- | --- | --- | ---------------------- | ----------------------------- |
| 文本字段 | テキスト読取 · 字段表     | マスク      | 开关  | 否   | 标记该账票内该字段为脱敏/打码对象（字段级） | 节点敏感情报检出读取；Step5 マスク対象；导出脱敏后值 |
| 表格列  | テーブル情報読取 · 各表列定义 | マスク      | 开关  | 否   | 标记该账票内该列为脱敏/打码对象（字段级）  | 同上                            |


界面文案：

- マスク列 i 提示：「マスキング（脱敏・打码）対象フィールドの指定。前処理の敏感情报检出が ON かつ対象帳票に含まれる場合のみ実行されます。」
...
与正常条件同表（テキスト読取 / テーブル情報読取 双 tab）。QR/OCR 写值均先经固定 処理ルール；后处理成功后，若该字段/列 処理ルール 允许配置且已配正常条件，则进入正常条件比较。

```
Step2 プロンプト（提示词） → 写值 → 固定処理ルール →（処理ルール适用时）正常条件比较（均不调大模型）
```
...
**运行时判定**（后处理成功后；**処理ルール** 为 マスタ照合 / テキスト置換、未配置正常条件或后处理产物为空则跳过）
...
4. **规则 1**（读取置信低于阈值，**须已勾选**）：仅 **OCR / QR→OCR 兜底** 路径；低置信 alone 即命中，与正常值是否越界无关（可与规则 6、7 叠加勾选）。
...
| 运行时 · 正常条件未通过 | 后处理越界（规则 6）；OCR·兜底低置信（规则 1）或低置信且越界（规则 7）；QR 写值越界（规则 6）                | `reviewRequired` → OCR 确认                                                                                       |
...
1. 读取模型弹窗确认规则扩展为七项（前五项为读取置信/Master 等既有规则，第六项为「正常値範囲を超えています」，第七项为「低信頼度かつ正常値範囲を超えています」）；**复选框多选 OR**；**默认勾选规则 6 + 7**；admin 可改为其他组合；**任一已勾选规则命中** → OCR 抽出确认待办（多选细则见下节）。
2. 画像不正 preprocessResult=reviewRequired → 前处理人工确认待办（非 OCR 抽出确认页）；待办/后端须携带不正类型（AI / 模版 / AI生成）。
3. 前处理确认页脱敏预览与字段展示（待确认）。
4. OCR 抽出确认执行页：要確認字段高亮 + 可编辑；**不展示**要確認原因、范围结论、配置区间、读取来源（读取来源仅账票类别设定 Step5 / 案件日志）。
5. QR 不单独产生待办类型；QR 与 OCR 路径汇合后按 **6.04 适用矩阵** 判定（QR 写值不适用规则 1、2、7；规则 6 为越界 alone）。
...
七项为 OR 关系：账票级任一字段命中 → 该账票合并为一条 OCR 抽出确认待办。
...
| ----- | ------ | --------------------------------------------- | --------------- | --------------- | -------------- | ------------- | -------------- | ---------- |
| OCR設定 | OCR写值  | Step2 选 OCR設定；或 QR設定 但无有效 QR 映射或QR检出失败出发OCR兜底 | 适用（OCR 置信）      | 适用              | 适用             | 适用            | 适用（低置信+越界）   | 七项任一命中     |
| QR設定  | QR 写值  | 对应 QR 扫描成功，映射写值完成                             | 不适用（无 OCR 读取置信） | 不适用（未走 OCR 多模型） | 适用             | 适用            | 不适用            | 规则 3～6 命中  |


说明：

- QR 成功写值路径**不产生 OCR 模型读取置信度**，规则 1、2、7 **不适用**；规则 **6 在 QR 写值上为正常条件越界 alone 即命中**。规则 2 亦仅 OCR / QR→OCR 兜底路径适用。
