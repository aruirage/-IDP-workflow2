...
| 正常值校验 | 账票 Step3 正常条件          | 后处理成功后比较（**全路径**）；规则 6：越界 alone；规则 7：OCR·兜底低置信且越界 → OCR 抽出确认（见 6.04）                        |
| 确认规则  | 读取模型弹窗七项               | **多选 OR**；默认规则 6 + 7；运行时**只评估已勾选**规则；适用矩阵见 6.04                                            |
...
- QR 兜底字段与纯 OCR 路径相同，规则 1、2、6、7 按 OCR 口径；全部 QR 未扫到即为所有已映射字段兜底，不单独设路由分支。
