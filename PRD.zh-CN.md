...
| 脱敏标记    | マスク            | Step2 字段/表格列级打码标记（与 HITL 并列的独立开关，默认 OFF）。ON 的字段/列在下游自动排除：Step3 処理ルール 不出现该字段/列（含 AI匹配草案与正常条件校验，残留错误自动清除）；Step5 効果テスト 文本卡片不显示、表格单元格显示 ***。与前处理节点敏感情报检出（账票级脱敏工程）的衔接待后续明确 |
...
| 账票类型设置  | 帳票タイプ設定        | 配置端账票主数据模块；Step2 含 OCR/QR 双模式、字段 HITL 与 マスク；Step3 含処理ルール与正常范围（マスク字段/列自动排除）；Step4 为前处理图像级设置（无字段级マスク）；Step5 効果テスト 按マスク结果打码 |
...
| マスク vs 敏感情报检出               | マスク（字段/列级）在账票类型 Step2 配置，决定下游 Step3/Step5 的字段排除与打码显示；敏感情报检出（节点）定账票级是否执行。本期对象账票 = Step1 关联账票；两者衔接待后续明确 |
...
| 配置 test vs 前处理确认预览          | Step5 効果テスト 按マスク结果排除文本卡片、表格单元格显示 ***；遮挡预览与打码在前处理确认页（字段级范围待后续明确） |
...
角色：admin（节点侧启用敏感情报检出）；操作员（查看脱敏结果；无专用待办）

目标：字段级マスク在账票类型 Step2 配置（与 HITL 并列的独立开关），下游 Step3/Step5 自动排除与打码。敏感情报检出仍在前处理节点以开关 + 对象账票决定账票级是否执行；对象账票可选范围为业务场景 Step1 关联账票。字段级打码与节点脱敏工程的衔接待后续明确。

配置流程：

```
业务场景 Step2 前処理設定：敏感情报检出 ON + 对象账票（账票级）
（字段级マスク：账票类型 Step2 各字段/列的マスク开关，默认 OFF）
```

1. admin 在业务场景 Step2 前処理設定 开启「敏感情报检出」，从 Step1 关联账票中选择对象账票。
2. 账票类型 Step2 マスク开关的下游效果：Step3 処理ルール 不出现该字段/列（含 AI匹配草案与正常条件校验）；Step5 効果テスト 文本卡片隐藏、表格单元格显示 ***。前处理确认页的打码预览与节点侧敏感情报检出衔接待后续明确。
...
1. 工作流进入前处理节点；当敏感情报检出 ON 且当前实例账票在对象账票列表中，执行账票级脱敏工程（字段级范围待 Step4）。
...
1. admin 在业务场景 Step2 前処理設定 开启「画像不正検知」。
...
3. Step5 test：每字段卡片右上角展示读取来源标签（QR読取 或 OCR読取，仅两种）；HITL=ON 且规则命中时输入框标红；HITL=OFF 字段不标要確認。
...
2. 敏感情报检出：模块 ON 且实例账票在对象列表时执行账票级脱敏。对象账票可选范围 = 业务场景 Step1 关联账票。字段级打码范围待 Step4 マスク明确后再衔接。
...
Step2：QR 仅字段读取tab适配；字段读取内切换 OCR設定 / QR設定；OCR 维护字段/列主数据 + HITL + 拖拽顺序/删减等；【AI自動生成】 仅 OCR 字段/列，不生成 QR 相关配置。マスク列与 HITL 并列配置；マスク=ON 的字段/列在 Step3/Step5 自动排除与打码。

Step3：字段读取 / 表格读取 新增正常条件列（範囲内、<、≤、>、≥、列挙）；读取模型弹窗七项确认规则见 6.04。

Step5：test 展示字段值、读取来源（QR読取 / OCR読取）、HITL=ON 且规则命中时输入框标红。本期 QR 扫描与映射以 Z 客户诊断书 template 为主。
...
4. 写值与后处理汇合后，确认路径顺序：HITL → 已勾选人工确认规则 →（仅当勾选规则 6/7 时）正常条件比较 → 规则 OR 判定；命中则将该 HITL=ON 字段纳入 OCR 抽出确认待办。HITL=OFF 或未勾选依赖正常条件的规则时，不做正常条件比较。
...
Step2 AI 读取分 テキスト読取 / テーブル情報読取 两个 tab；HITL 与 マスク 列均在 OCR設定 下配置。テーブル情報読取 固定 OCR（无 QR 模式切换），按各表格卡片维护列定义（各列含 必須/HITL/マスク 开关）；マスク=ON 的列在 Step3/Step5 自动排除与打码。
...
| マスク反映   | 适用     | 适用       | Step2 マスク=ON 的字段不显示卡片；表格列单元格固定显示 *** 且不标红 |
