...
| 脱敏标记    | マスク            | 字段/列级脱敏与打码对象标记；配置入口改至账票类型 Step4。本期要做，但需求未明确，故先不加配置。原 Step2 マスク列已替换为 HITL |
| 敏感情报检出  | 敏感情報検出         | 前处理节点子模块：模块开关 + 对象账票（账票级）。对象账票可选范围暂为业务场景 Step1 关联账票；字段级マスク改至 Step4；本期要做，但需求未明确，故先不加字段级配置，明确后再与节点衔接 |
| 分類信頼度閾値 | 分類信頼度閾値        | Step1 必填项：案件集约识别分类阶段的账票分类置信度下限（百分比）                                                                                                              |
| 账票类型设置  | 帳票タイプ設定        | 配置端账票主数据模块；Step2 含 OCR/QR 双模式与字段 HITL；Step3 含処理ルール与正常范围；Step4 预留マスク（本期要做，需求未明确故先不加） |
...
| 敏感情报检出 vs 画像不正検知            | 脱敏为前处理工程（账票级开关 + 对象账票；字段级マスク改至 Step4；本期要做，需求未明确故先不加）；画像不正为一级模块开关 + 二级检测（PS / AI生成 / 帳票特徴 / 筆跡，各自开关与对象账票分开选，运行时四项并行），不通过时返回不正类型 → 前处理人工确认待办 |
| マスク vs 敏感情报检出               | マスク（字段/列级）配置入口改至 Step4（本期要做，需求未明确故先不加）；敏感情报检出（节点）定账票级是否执行。本期对象账票 = Step1 关联账票；字段级打码范围待 Step4 |
...
| 配置 test vs 前处理确认预览          | Step5 本期不标记脱敏对象（字段级マスク改至 Step4）；遮挡预览与打码在前处理确认页（字段级范围待 Step4） |
...
目标：字段级マスク配置改至账票类型 Step4。本期要做，但需求未明确，故先不加配置。敏感情报检出仍在前处理节点以开关 + 对象账票决定账票级是否执行；对象账票可选范围为业务场景 Step1 关联账票。字段级打码范围待 Step4 明确后再衔接。

配置流程：

```
业务场景 Step2 前処理設定：敏感情报检出 ON + 对象账票（账票级）
（字段级マスク：Step4；本期要做，需求未明确故先不加）
```

1. admin 在业务场景 Step2 前処理設定 开启「敏感情报检出」，从 Step1 关联账票中选择对象账票。
2. Step5 / 前处理确认页的字段打码预览：本期不依赖 Step2 列配置（原マスク列已改为 HITL）。
...
| 2～4 | 回転 → 補正 → 整列 | 几何预处理                                    |
| 5   | 敏感情报检出       | 几何完成后对对象账票执行脱敏工程；字段级マスク改至 Step4（本期要做，需求未明确故先不加）；OCR 读处理后实例；无业务待办 |
...
Step2：QR 仅字段读取tab适配；字段读取内切换 OCR設定 / QR設定；OCR 维护字段/列主数据 + HITL + 拖拽顺序/删减等；【AI自動生成】 仅 OCR 字段/列，不生成 QR 相关配置。原マスク列已替换为 HITL；字段级マスク改至 Step4（本期要做，需求未明确故先不加）。
...
Step2 AI 读取分 テキスト読取 / テーブル情報読取 两个 tab；HITL 列均在 OCR設定 下配置。テーブル情報読取 固定 OCR（无 QR 模式切换），按各表格卡片维护列定义。原マスク列已从此处移除，改至 Step4（本期要做，需求未明确故先不加）。
...
3. 前处理确认页脱敏预览与字段展示（字段级マスク改至 Step4；本期要做，需求未明确故先不加）。
