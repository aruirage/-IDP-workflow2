...
| 抽出单位    | （プロンプト（提示词）约定） | Step2 プロンプト（提示词）中声明的 OCR **输出计量单位**（如 cm、円、日）；与 字段类型 强绑定；**正确答案**与 Step3 正常条件阈值均使用该量纲下的纯数字                                                       |
...
| 敏感情报检出 vs 画像不正検知            | 脱敏为前处理工程（读取对象账票的 マスク 配置并打码）；画像不正为模型二值判定（通过/不通过）→ 前处理人工确认待办；二者均无阈值配置                                                                                                                    |
...
| タイプ（字段类别） vs プロンプト（提示词）抽出单位 | **タイプ（字段类别）**（`number` / `enum` / `string`）声明字段类别与 OCR 数据形态；**cm / m / 円** 等量纲写在 **プロンプト（提示词）**，不由 タイプ（字段类别）表达；正解サンプル须按 プロンプト（提示词） 量纲填写（如账面 `1.6米`、プロンプト（提示词） 约定 cm → 正解 `160`）；**正常条件可否配置由 処理ルール 决定，不由 タイプ（字段类别）决定** |
...
```
配置：Step3 启用正常条件 + 読取モデル設定 人工確認ルール（**默认仅规则 6**，可追加 1～5）→ Step5 test 验收
...
2. 运行时逐字段比较后处理产物（**OCR、QR 写值、QR→OCR 兜底均适用**）；**仅评估读取模型弹窗中已勾选**的人工确认规则，**OR** 合并——**默认仅规则 6** 时：**QR 写值**越界 alone 即进确认；**OCR / 兜底**须低置信且越界。若 admin 另启规则 1，则 OCR 低置信 alone 亦可进确认（组合见 6.04 · 多选配置）。
...
1. 工作流进入前处理节点；**当**敏感情报检出 ON **且**当前实例账票在对象账票列表中，读取该账票 Step2 マスク=ON 的字段/列并执行脱敏与打码。
...
2. 依赖账票 Step1 template 与客户正常样张库做模板特征比对，PS和AI生成依赖开源算法（待确定）判断是否伪造。
...
| 2   | 敏感情报检出       | 对对象账票读取 Step2 マスク 配置并打码；OCR 读脱敏后实例；无业务待办 |
| 3～5 | 回転 → 補正 → 整列 | 几何预处理                                    |


文件状态四段枚举不变；子模块进度汇总至 preprocessStatus / preprocessResult，不在案件一覧展示子模块名。

1. 前处理面板新增画像不正检测、敏感情报检出（各：开关 + 对象账票）；插入在画像回転/補正/整列之前。
...
| 确认规则  | 读取模型弹窗六项               | **多选 OR**；默认仅规则 6；运行时**只评估已勾选**规则；适用矩阵见 6.04                                            |
...
| 按钮     | 界面文案（日语） | 显示条件                   | 点击后                                                                                                                 |
| ------ | -------- | ---------------------- | ------------------------------------------------------------------------------------------------------------------- |
| AI自动生成 | AI自動生成   | 字段读取 · OCR設定，或表格读取 tab | 生成 OCR プロンプト（提示词）と正解サンプル（字段/列级）；プロンプト（提示词） 须含**抽出位置、输出格式、抽出单位**（含账面单位与输出单位不一致时的换算说明）；正解サンプル 与 プロンプト（提示词） 同量纲；不涉及 QR |
| AI自动生成 | —        | 字段读取 · QR設定            | 不显示                                                                                                                 |


#### Step2 プロンプト（提示词）

**タイプ（字段类别） 与 プロンプト（提示词） 的分工**

- **タイプ（字段类别）**（Step2 項目タイプ）：声明字段**类别**——`string` / `number` / `enum` 等；决定 OCR 读取与后处理时的**数据形态**。**タイプ（字段类别）不承载计量单位**，**不单独决定** Step3 正常条件可否配置（由 **処理ルール** 决定）。
- **プロンプト（提示词）**：在 タイプ（字段类别）已确定类别的前提下，约束读取模型——**从账票何处抽取、以何种格式输出、结果的计量单位（抽出单位）是什么**；若账面上的写法/单位与约定输出单位不一致，须在 プロンプト（提示词） 写明换算规则（由读取模型在抽取阶段完成，**运行时不另调大模型做单位推断**）。
...
| ---- | -------------- | -------------- | ------------------------------------------ |
| 文本字段 | テキスト読取 · 字段表   | プロンプト          | 约束读取模型：抽取位置、输出格式、**抽出单位**；界面列名 プロンプト = 提示词 |
| 文本字段 | 同上             | 正解サンプル         | 与 プロンプト（提示词） 同量纲、同格式的样例值，供 test / 调优       |
| 表格列  | テーブル情報読取 · 列定义 | プロンプト / 正解サンプル | 同上；列级 プロンプト = 提示词                          |


**プロンプト（提示词） 须写清的内容**

`number` 类字段（タイプ（字段类别）=`number`）：

1. **抽取位置**：账票上读哪个栏位/语义（如「身長欄」「請求金額欄」）。
2. **抽出单位**：输出值采用的计量单位（如 cm、円、日）；**写在 プロンプト（提示词），不写在 タイプ（字段类别）**。
3. **单位换算**（若账面与抽出单位不一致）：须在 プロンプト（提示词） 明示。例：账面 `1.6米` / `1.6m`，プロンプト（提示词） 约定输出单位为 cm → 模型输出 `160`（不带 `cm` 后缀时写「単位は含めない」）。
4. **输出格式**：半角数字、是否允许小数、输出是否带单位后缀（number 类通常输出**纯数字**，量纲仅由 プロンプト（提示词） 约定、正解サンプル 与 Step3 阈值体现）。

`enum` / `string` 类字段：プロンプト（提示词） 约定输出形态（如原文照录、仅输出枚举值之一、日期格式等）；**正解サンプル** 同样须与 プロンプト（提示词） 一致。
...
- **OCR / QR→OCR 兜底**：读取时消费 Step2 プロンプト（提示词）（含抽出单位与换算）；写值后进入 Step3 固定规则链。
...
| 阶段               | 职责                                     | 是否调大模型      |
| ---------------- | -------------------------------------- | ----------- |
| Step2 プロンプト（提示词） | 约定 OCR **抽取输出**的格式、**抽出单位**及账面→约定单位的换算 | 读取阶段由读取模型执行 |
| Step3 処理ルール      | 按 admin 所选固定规则做格式标准化（不再推断/换算量纲）        | **否**       |
| Step3 正常条件       | 数值范围或列挙比较；阈值为**与 プロンプト（提示词） 同量纲的纯数字**  | **否**       |


**配置对齐链**（number 类）

```text
タイプ（字段类别）=number → プロンプト（提示词） 写明抽出单位（+ 换算） → 正解サンプル 同量纲
...
- 全文（OFF）：QR 扫到 → 整串写值。
- 分隔（ON）：QR 扫到 → 切分后取对应段写值；段缺失/为空视为该字段 QR 未取到值，该字段走 **OCR 兜底**（若已配置 QR 映射）。
...
| **マスタ照合** | **不展示、不配、不参与** | — |
| **テキスト置換** | **不展示、不配、不参与** | — |
| 其余所有 処理ルール | 可配 | **範囲内 / < / ≤ / > / ≥ / 列挙**（六种模式均可选，**不**按 Step2 タイプ 或 処理ルール 类别限制） |


**バーコード等不确定格式字段**：処理ルール 选 バーコード 等时，可用 **列挙** 列出允许取值，亦可选数值比较模式（若后处理产物可解析为数字）。
...
2. **规则 6**（→ OCR 抽出确认，**须已勾选**）按取值路径分流：
  - **QR 写值**：Step3 正常条件越界 alone 即命中（无 OCR 读取置信度，不要求「低置信」）。
  - **OCR / QR→OCR 兜底**：OCR 读取置信低于阈值 **且** 正常条件越界，同时成立才命中。
3. **规则 1**（读取置信低于阈值，**须已勾选**）：仅 **OCR / QR→OCR 兜底** 路径；低置信 alone 即命中，与正常值是否越界无关（可与规则 6 叠加勾选）。
4. QR 写值路径：规则 1、2 不适用（无 OCR 读取置信 / 未走 OCR 多模型）；规则 3～6 在已勾选时适用。
...
| 后处理产物**为空** | **跳过**正常条件比较 |
| 比较失败 alone | **不单独**生成 OCR 抽出确认待办；须结合 6.04 已勾选规则（默认规则 6：QR 写值越界 alone；OCR/兜底低置信且越界） |
...
| 结果                | 取值路径     | 典型示例                                   |
| ----------------- | -------- | -------------------------------------- |
| 规则 6 命中（默认配置）     | OCR / 兜底 | 通院日数 OCR 置信 62%，后处理值 `20` 超出 `1`～`15`  |
| 规则 6 命中（默认配置）     | QR 写值    | QR 读得通院日数 `20`，超出 `1`～`15`（不要求 OCR 置信） |
| 仅越界（默认配置：仅规则 6 时） | OCR / 兜底 | 通院日数置信正常但值 `20` 超出配置区间 → **不进**待办      |
| 仅低置信（须启用规则 1）     | OCR / 兜底 | 某字段 OCR 置信 62%，正常条件通过 → **进**待办        |
...
1. 读取模型弹窗确认规则扩展为六项（前五项为读取置信/Master 等既有规则，第六项为「低信頼度かつ正常値範囲を超えています」）；**复选框多选 OR**；**默认仅勾选规则 6**；admin 可追加 1～5 或改为其他组合；**任一已勾选规则命中** → OCR 抽出确认待办（多选细则见下节）。
2. 画像不正 preprocessResult=reviewRequired → 前处理人工确认待办（非 OCR 抽出确认页）。
...
- QR 兜底字段与纯 OCR 路径相同，规则 1、2、6 按 OCR口径（规则 6 须低置信且越界）；全部 QR 未扫到即为所有已映射字段兜底，不单独设路由分支。
