...
| 脱敏标记    | マスク            | Step2 字段/表格列级打码标记（与 HITL 并列的独立开关，默认 OFF）。ON 的字段/列不参与 OCR 读取（无读取值）：Step3 処理ルール 不出现该字段/列（含 AI匹配草案与正常条件校验，残留错误自动清除）；Step5 効果テスト 不展示。前处理确认页的打码按本标记给相应字段打码 |
...
| マスク vs 敏感情报检出               | マスク（字段/列级）在账票类型 Step2 配置，ON 的字段/列不参与 OCR 读取，下游 Step3/Step5 直接排除（不展示）；敏感情报检出（节点）定账票级是否执行。本期对象账票 = Step1 关联账票；两者衔接待后续明确 |
...
2. 敏感情报检出：模块 ON 且实例账票在对象列表时执行账票级脱敏。对象账票可选范围 = 业务场景 Step1 关联账票。字段级打码范围见账票类型 Step2 マスク开关（脱敏列）；与节点脱敏工程的衔接待后续明确。
...
与敏感情报检出（前处理节点）的关系：マスク 是字段/列级配置（账票类型 Step2），敏感情报检出是账票级脱敏工程（业务场景 Step2 前処理設定，开关 + 对象账票）；本期对象账票 = Step1 关联账票，两者衔接待后续明确。
...
配置端 mock；范围结论**仅 Step5** 展示，运行时确认页与案件详情不展示。默认以内置サンプル帳票（診断書）直接展示结果；「テスト」可手动重跑，「ファイル選択」选择新文件后自动重跑。


| 输出项    | テキスト読取 | テーブル情報読取 | 说明                                                    |
| ------ | ------ | -------- | ----------------------------------------------------- |
| 字段值    | 适用     | 适用（表格行）  | 只读展示 mock 读取结果                                        |
| 范围结论   | 适用     | 适用       | **仅 Step5**：範囲OK / 超出正常值范围 + 配置区间（如 `1`～`15`）；超范围结论仅在已勾选范围类规则（规则 6/7）命中时标红显示，未勾选则不显示该行 |
